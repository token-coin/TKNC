...
/** Call node.updateRwSetting() with TKNC 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because TKNC 22.x releases try to read these specific settings as
        ...
        // instead of strings, because tknc 22.x will not try to read these.
...
//! Convert enabled/size values to tknc -prune setting.
...
//! Get pruning enabled value to show in GUI from tknc -prune setting.
...
//! Get pruning size value to show in GUI from tknc -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayUnit")) {
        settings.setValue("DisplayUnit", QVariant::fromValue(TKNCUnit::TKNC));
    }
    QVariant unit = settings.value("DisplayUnit");
    if (unit.canConvert<TKNCUnit>()) {
        m_display_tknc_unit = unit.value<TKNCUnit>();
    } else {
        m_display_tknc_unit = TKNCUnit::TKNC;
        settings.setValue("DisplayUnit", QVariant::fromValue(m_display_tknc_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if tknc.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_tknc_unit);
...
{
    if (new_unit.isNull() || new_unit.value<TKNCUnit>() == m_display_tknc_unit) return;
    m_display_tknc_unit = new_unit.value<TKNCUnit>();
    QSettings settings;
    settings.setValue("DisplayUnit", QVariant::fromValue(m_display_tknc_unit));
    Q_EMIT displayUnitChanged(m_display_tknc_unit);
}

...
static void RegisterMetaTypes()
{
 ...
 qRegisterMetaType<TKNCUnit>("TKNCUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
 ...
 // Load e.g. tknc_de.qm (shortcut "de" needs to be defined in tknc.qrc)
 if (translatorBase.load(lang, ":/translations/")) {
 QApplication::installTranslator(&translatorBase);
 }

 // Load e.g. tknc_de_DE.qm (shortcut "de_DE" needs to be defined in tknc.qrc)
...
static const char* qt_argv = "tknc-qt";

TKNCApplication::TKNCApplication()
...
void TKNCApplication::setupPlatformStyle()
{
 // UI per-platform customization
 // This must be done inside the TKNCApplication constructor, or after it, because
 // PlatformStyle::instantiate requires a QApplication
 std::string platformName;
 platformName = gArgs.GetArg("-uiplatform", TKNCGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void TKNCApplication::createPaymentServer()
{
 paymentServer = new PaymentServer(this);
}
#endif

bool TKNCApplication::createOptionsModel(bool resetSettings)
{
...
void TKNCApplication::createWindow(const NetworkStyle *networkStyle)
{
 window = new TKNCGUI(node(), platformStyle, networkStyle, nullptr);
 connect(window, &TKNCGUI::quitRequested, this, &TKNCApplication::requestShutdown);
...
void TKNCApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
 assert(!m_splash);
...
void TKNCApplication::createNode(interfaces::Init& init)
{
 assert(!m_node);
...
bool TKNCApplication::baseInitialize()
{
 return node().baseInitialize();
}

void TKNCApplication::startThread()
{
 assert(!m_executor);
 m_executor.emplace(node());

 /* communication to and from thread */
 connect(&m_executor.value(), &InitExecutor::initializeResult, this, &TKNCApplication::initializeResult);
 connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
 QCoreApplication::exit(0);
 });
 connect(&m_executor.value(), &InitExecutor::runawayException, this, &TKNCApplication::handleRunawayException);
 connect(this, &TKNCApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
 connect(this, &TKNCApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void TKNCApplication::parameterSetup()
{
...
void TKNCApplication::InitPruneSetting(int64_t prune_MiB)
{
 optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void TKNCApplication::requestInitialize()
{
...
void TKNCApplication::requestShutdown()
{
...
void TKNCApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
 ...
 // tknc: URIs or payment requests:
 if (paymentServer) {
 connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TKNCGUI::handlePaymentRequest);
 connect(window, &TKNCGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void TKNCApplication::handleRunawayException(const QString &message)
{
...
void TKNCApplication::handleNonFatalException(const QString& message)
{
...
WId TKNCApplication::getMainWinId() const
{
 if (!window)
 return 0;
...
bool TKNCApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
 ...
 argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", TKNCGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
 ...
 tkncsignals::scoped_connection handler_message_box = ::uiInterface.ThreadSafeMessageBox_connect(noui_ThreadSafeMessageBox);
 tkncsignals::scoped_connection handler_question = ::uiInterface.ThreadSafeQuestion_connect(noui_ThreadSafeQuestion);
 tkncsignals::scoped_connection handler_init_message = ::uiInterface.InitMessage_connect(noui_InitMessage);

 // Do not refer to data directory yet, this can be overridden by Intro::pickDataDirectory

 /// 1. Basic Qt initialization (not dependent on parameters or configuration)
 Q_INIT_RESOURCE(tknc);
 Q_INIT_RESOURCE(tknc_locale);
...
#ifdef ENABLE_WALLET
 if (arg.startsWith(TKNC_IPC_PREFIX, Qt::CaseInsensitive)) {
 invalid_token &= false;
 ...
 if (invalid_token) {
 InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see tknc-qt -h for a list of options.", argv[i])));
 QMessageBox::critical(nullptr, CLIENT_NAME,
 // message cannot be translated because translations have not been initialized
 QString::fromStdString("Command line contains unexpected token '%1', see tknc-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
 return EXIT_FAILURE;
 }
 }
 ...
 /// 6-7. Parse tknc.conf, determine network, switch to network specific
 ...
 // tknc: links repeatedly have their payment requests routed to this process:
 if (WalletModel::isWalletEnabled()) {

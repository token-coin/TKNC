...
#include <qt/tkncaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TKNC address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TKNCAddressEntryValidator(parent));
    widget->setCheckValidator(new TKNCAddressCheckValidator(parent));
}
...
bool parseTKNCURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no tknc: URI
    if(!uri.isValid() || uri.scheme() != QString("tknc"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!TKNCUnits::parse(TKNCUnit::TKNC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseTKNCURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTKNCURI(uriInstance, out);
}

QString formatTKNCURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("tknc:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TKNCUnits::format(TKNCUnit::TKNC, info.amount, false, TKNCUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openTKNCConf()
{
    ...
    /* Open tknc.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TKNC.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TKNC (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("TKNC (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "tknc.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("tknc-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a tknc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=TKNC\n";
        else
            optionFile << strprintf("Name=TKNC (%s)\n", ChainTypeToString(chain));

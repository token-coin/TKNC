...
/** Time window to wait between writing blocks/block index and chainstate to disk. Randomize writing time to prevent the network from settling into synchronized writer cohorts. */
...
/** The number of blocks to keep below the deepest prune lock. Higher than typical mainnet reorgs but not so high as to interfere with pruning. */
...
bool CheckFinalTxAtTip(const CBlockIndex& active_chain_tip, const CTransaction& tx)
{
    ...
    // CheckFinalTxAtTip uses active_chain_tip.Height()+1 because IsFinalTx called inside AcceptBlock uses the height of the block being evaluated. To check if a tx can be in the next block, pass tip+1.
    const int nBlockHeight = active_chain_tip.nHeight + 1;

    // BIP113 requires nLockTime < median time of the previous block. The next block's previous block is the current tip, so use that median time.
...
namespace {
/** A helper which calculates heights of inputs of a given transaction. @param[in] tip current chain tip (mempool inputs assumed to confirm in the next block); @param[in] coins view of relevant coins; @param[in] tx transaction being evaluated. @returns input heights, or nullopt on error. */
    ...
    const CTransaction& tx)
{
    ...
    next_tip.pprev = tip;
    // When SequenceLocks is called inside ConnectBlock, the height of the block being evaluated is used. To check if a tx can be in the next block, use tip+1.
    next_tip.nHeight = tip->nHeight + 1;
    const auto [min_height, min_time] = CalculateSequenceLocks(tx, STANDARD_LOCKTIME_VERIFY_FLAGS, prev_heights.value(), next_tip);

    // Also store the hash of the block with the highest height of all the blocks which have sequence locked prevouts. This hash needs to still be on the chain for these LockPoint calculations to be valid. Note: It is impossible to correctly calculate a maxInputBlock if any of the sequence locked inputs depend on unconfirmed txs, except in the special case where the relative lock time/height is 0 (equivalent to no sequence lock). We assume input height of tip+1 for mempool txs and test the resulting min_height and min_time from CalculateSequenceLocks against tip+1.
    ...
    // tip->GetAncestor(max_input_height) should never return nullptr because max_input_height is always less than the tip height. It would be a bad bug to continue, since a LockPoints with maxInputBlock=nullptr signifies no relative lock time.
                             ...
                             const LockPoints& lock_points)
{
    ...
    CBlockIndex index;
    index.pprev = tip;
    // CheckSequenceLocksAtTip uses active_chainstate.m_chain.Height()+1 for height-based locks: SequenceLocks called inside ConnectBlock uses the height of the block being evaluated. To check if a tx can be in the next block, use tip+1.
    ...
    bool fAddToMempool)
{
    ...
    {
        // disconnectpool is ordered so the front is the most recently-confirmed tx (the last tx of the block at the tip) in the disconnected chain. Iterate disconnectpool in reverse to add txs back to the mempool starting with the earliest tx previously seen in a block.
    ...
    // AcceptToMemoryPool/addNewTransaction assume new mempool entries have no in-mempool children—generally not true when re-adding previously-confirmed txs. UpdateTransactionsFromBlock finds descendants of re-added disconnectpool txs and cleans up mempool state.
    m_mempool->UpdateTransactionsFromBlock(vHashUpdate);

    // Predicate for filtering txs in removeForReorg. Checks finality and coinbase maturity, and updates valid entries' cached LockPoints. If false, tx stays valid and lockpoints are updated. If true, tx is invalid in the next block—remove it and all descendants. Note: TRUC rules are not applied here, so reorgs may cause TRUC inheritance or topology violations.
...
/** Avoids mempool polluting consensus critical paths—cached sig and script validity results are reused if the tx is revalidated during block validation. */
                ...
                EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        // If the Coin is available, 2 possibilities: in our current ChainstateActive UTXO set, or a UTXO from a tx in our mempool. Ensure the scriptPubKeys in Coins from CoinsView are correct.
...
public:
    ...
    struct ATMPArgs {
        ...
        /*
         * Outpoints added to the coins cache as a result of mempool tx validation (not previously present). The caller may remove them if the tx is rejected.
         */
        std::vector<COutPoint>& m_coins_to_uncache;
        /** When true, the transaction or package will not be submitted to the mempool. */
        const bool m_test_accept;
        /** Whether we allow transactions to replace mempool transactions. If false, any tx spending the same inputs as a mempool tx is a conflict. */
        ...
        /** Skip LimitMempoolSize() within AcceptSingleTransaction(). Used when multiple AcceptSubPackage calls are expected and mempool is trimmed at the end of AcceptPackage(). */
        const bool m_package_submission;
        /** When true, use package feerates instead of individual tx feerates for fee-based policies (mempool min fee, min relay fee). */
        const bool m_package_feerates;
        /** Catches absurd fees from local submission due to wallet fee miscalculation. std::nullopt = unset (allow any feerates). Any tx failing this causes immediate failure. */
    ...
    /** Multiple transaction acceptance. Transactions may or may not be interdependent, but must not conflict with each other, and cannot already be in the mempool. Parents must come before children if any dependencies exist. */
    ...
    /** Submission of a subpackage. If size==1, calls AcceptSingleTransaction with adjusted ATMPArgs to enable sibling eviction and wraps the result. If size>1, calls AcceptMultipleTransactions. Cleans up all non-chainstate coins from m_view at the end. */
    PackageMempoolAcceptResult AcceptSubPackage(const std::vector<CTransactionRef>& subpackage, ATMPArgs& args)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_pool.cs);

    /** Package (more specific than multiple transactions) acceptance. Package must be a child with all unconfirmed parents, topologically sorted. */
...
private:
    ...
    struct Workspace {
        ...
        /** Txids of mempool transactions that this transaction directly conflicts with or may replace via sibling eviction. */
        std::set<Txid> m_conflicts;
        /** Iterators to mempool entries that this transaction directly conflicts with or may replace via sibling eviction. */
        ...
        /** Whether RBF-related data structures (m_conflicts, m_iters_conflicting, m_replaced_transactions) include a sibling in addition to txns with conflicting inputs. */
        bool m_sibling_eviction{false};

        /** Virtual size as used by the mempool, calculated from serialized size and sigops. */
        int64_t m_vsize;
        ...
        /** If doing package validation (m_package_feerates=true), the "effective" package feerate is total fees / total size of txs (may include ancestors and/or descendants). */
    ...
    // Submit all transactions to the mempool and call ConsensusScriptChecks to add to the script cache—only after successful validation of all txs in the package. Does not call LimitMempoolSize(), so mempool max_size_bytes may be temporarily exceeded.
...
private:
    ...
    /** Cached view of available coins from UTXO set, mempool, and artificial temporary coins (for package validation). The view doesn't track spent coins; conflicts are detected in other ways: within a tx via CheckTransaction (bad-txns-inputs-duplicate), within a package via IsWellFormedPackage (conflict-in-package), with mempool via CTxMemPool::GetConflictTx (replacements allowed). Temporary coins persist between tx checks so package validation is possible, but must be cleaned up when a subpackage finishes (accepted or rejected). The cache must also be cleared when mempool contents change (changeset applied, mempool trims itself) because it can return cached coins that no longer exist. Use CleanupTemporaryCoins() when done with a SubPackageState or call LimitMempoolSize(). */
    CCoinsViewCache m_view;

    // These are the two possible backends for m_view.
    /** When m_view is connected to m_viewmempool as its backend, it can pull coins from the mempool and UTXO set. This is also where temporary coins are stored. */
    ...
    struct SubPackageState {
        ...
        // RBF-related members
        /** Whether the transaction(s) would replace any mempool transactions and/or evict any siblings. If so, RBF rules apply. */
        bool m_rbf{false};
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Only accept nLockTime-using transactions that can be mined in the next block; we don't want our mempool filled with txs that can't be mined yet.
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        // Note: this call may add txin.prevout to the coins cache (coins_cache.cacheCoins) via FetchCoin(). It should be removed later (via coins_to_uncache) if the tx turns out to be invalid.
    ...
    // All required inputs are cached now, so switch m_view to the empty backend. This keeps already-fetched cache entries for later checks and prevents new backend lookups (which would avoid coins_to_uncache tracking).
    m_view.SetBackend(CoinsViewEmpty::Get());

    assert(m_active_chainstate.m_blockman.LookupBlockIndex(m_view.GetBestBlock()) == m_active_chainstate.m_chain.Tip());

    // Only accept BIP68 sequence-locked transactions that can be mined in the next block; we don't want our mempool filled with txs that can't be mined yet. Pass in m_view with all relevant inputs cached; since m_view's backend was removed, it no longer pulls coins from the mempool.
    ...
    if (!args.m_bypass_limits) {
        ...
        if (const auto err{SingleTRUCChecks(m_pool, ws.m_ptx, ws.m_parents, ws.m_conflicts, ws.m_vsize)}) {
            ...
            if (args.m_allow_sibling_eviction && err->second != nullptr) {
                ...
                // Adding the sibling to m_iters_conflicting here means that it doesn't count towards RBF Carve Out above. This is correct, since removing to-be-replaced transactions from the descendant count is done separately in SingleTRUCChecks for TRUC transactions.
                ws.m_iters_conflicting.insert(m_pool.GetIter(err->second->GetHash()).value());
                ws.m_sibling_eviction = true;
                // The sibling will be treated as part of the to-be-replaced set in ReplacementChecks. Note that we are not checking whether it opts in to replaceability via BIP125 or TRUC (normally done in PreChecks). However, the only way a TRUC transaction can have a non-TRUC and non-BIP125 descendant is due to a reorg.
            } else {
                                     ...
                                     PackageValidationState& package_state)
{
    ...
    // If the package has in-mempool parents, we won't consider a package RBF since it would result in a cluster larger than 2. N.B. To relax this constraint we will need to revisit how CCoinsViewMemPool::PackageAddTransaction is used inside AcceptMultipleTransactions to track available inputs while processing a package. Specifically we'd need to check that the ancestors of the new txs don't intersect with the set of txs to be removed due to RBF, which is not checked at all in the package acceptance context.
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // Re-check against the current block tip's script verify flags to cache our script execution flags. Useless if the next block has different script flags, but the cache tracks flags and auto-invalidates—we just get a few extra misses on soft-fork activation. Also useful against bugs in standard flags that pass invalid txs (e.g. STRICTENC incorrectly allowing certain CHECKSIG NOT scripts). A similar check in CreateNewBlock (via TestBlockValidity) prevents building invalid blocks; allowing them into the mempool can be exploited as a DoS attack.
                                  ...
                                  std::map<Wtxid, MempoolAcceptResult>& results)
{
    ...
    // ConsensusScriptChecks adds to the script cache and is therefore consensus-critical; CheckInputsFromMempoolAndCache asserts that transactions only spend coins available from the mempool or UTXO set. Submit each transaction to the mempool immediately after calling ConsensusScriptChecks to make the outputs available for subsequent transactions.
    for (Workspace& ws : workspaces) {
    ...
    if (!all_submitted) {
        ...
        // This code should be unreachable; it's here as belt-and-suspenders to try to ensure we have no consensus-invalid transactions in the mempool.
        m_subpackage.m_changeset->Apply();
        m_subpackage.m_changeset.reset();
        return false;
    }
...
MempoolAcceptResult MemPoolAccept::AcceptSingleTransactionInternal(const CTransactionRef& ptx, ATMPArgs& args)
{
    ...
    if (ws.m_conflicts.size()) {
        ...
        // A transaction that spends outputs that would be replaced by it is invalid. Now that we have the set of all ancestors we can detect this pathological case by making sure ws.m_conflicts and this tx's ancestors don't intersect.
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactionsInternal(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    for (Workspace& ws : workspaces) {
        ...
        // Make the coins created by this transaction available for subsequent transactions in the package to spend. If there are no conflicts within the package, no transaction can spend a coin needed by another transaction in the package. We also need to make sure that no package tx replaces (or replaces the ancestor of) the parent of another package tx. As long as we check these two things, we don't need to track the coins spent. If a package tx conflicts with a mempool tx, PackageRBFChecks() ensures later that any package RBF attempt has *no* in-mempool ancestors, so we don't have to worry about subsequent transactions in same package spending the same in-mempool outpoints. This needs to be revisited for general package RBF.
...
{
    // 3 kinds of coins in m_view: (1) Temporary coins from subpackage txs, constructed by m_viewmempool; (2) Mempool coins from mempool txs, constructed by m_viewmempool; (3) Confirmed coins from current UTXO set. (1) Temporary coins need removal regardless of whether the tx was submitted: if submitted, m_viewmempool fetches them from there; if not, keeping them is incorrect (future calls may try to spend coins that don't exist). (2) Mempool coins also need removal: if mempool changed due to submission/replacement, previously-fetched coins may be spent/nonexistent and must be deleted from m_view. (3) Confirmed coins don't need removal: chainstate hasn't changed (we hold cs_main, no blocks processed), so the confirmed tx can't disappear. It may now be spent after submission, but we've checked the package has no 2 txs spending the same coin, and we check mempool conflicts via CTxMemPool::GetConflictTx. Keeping them avoids re-fetching confirmed coins later.
...
PackageMempoolAcceptResult MemPoolAccept::AcceptSubPackage(const std::vector<CTransactionRef>& subpackage, ATMPArgs& args)
{
    ...
    // Clean up m_view and m_viewmempool so that other subpackage evaluations don't have access to coins they shouldn't. Keep some coins in order to minimize re-fetching coins from the UTXO set. Clean up package feerate and rbf calculations
    ClearSubPackageState();
...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package& package, ATMPArgs& args)
{
    ...
    // Two topologies handled by this function: (1) a single transaction, (2) a child-with-parents package. Check that the package is well-formed; if not, we won't try to validate any txs and return a package-wide error.
    ...
    if (package.size() > 1 && !IsChildWithParents(package)) {
        // All transactions in the package must be a parent of the last transaction. This is just an opportunity for us to fail fast on a context-free check without taking the mempool lock.
    ...
    // Results from individual validation returned if no other result is available. "Nonfinal" because if a tx fails by itself but succeeds later (e.g. with a fee-bumping child), the result may be discarded.
    ...
    for (const auto& tx : package) {
        ...
        if (m_pool.exists(wtxid)) {
            // Exact transaction already exists in mempool. Node operators set their own mempool policies, nodes may receive txs in different orders, and malicious counterparties may exploit policy differences to pin/delay propagation. So some package txs may already be in the mempool—don't reject the entire package (censorship vector). De-duplicate already-in-mempool txs and only call AcceptMultipleTransactions with new ones; avoids double-counting counts/sizes (ancestor/descendant limits) and fees.
            ...
            // Transaction with the same non-witness data but different witness (same txid, different wtxid) already exists in the mempool. We don't allow replacement yet, so just swap the package tx for the mempool one. Note: ignoring the validity of the package tx passed in.
            ...
            if (single_res.m_result_type == MempoolAcceptResult::ResultType::VALID) {
                ...
                // Package validation policy differs from individual policy only in feerate evaluation. If a tx fails here for a consensus rule, the result won't change as part of a package. To minimize repeated work, unless the tx fails due to feerate or missing inputs (parent is a previous tx in the package that failed for feerate), don't run package validation. This decision may not make sense if different package types are allowed in the future. Continue individually validating the rest of the txs—some may still be valid.
                quit_early = true;
    ...
    // This is invoked by AcceptSubPackage() already, so this is just here for clarity (since it's not permitted to invoke LimitMempoolSize() while a changeset is outstanding).
    ClearSubPackageState();

    // Make sure we haven't exceeded max mempool size. Package transactions that were submitted to mempool or already in mempool may be evicted. If mempool contents change, the m_view cache is dirty (already cleared above).
    LimitMempoolSize(m_pool, m_active_chainstate.CoinsTip());
    ...
    for (const auto& tx : package) {
        ...
        if (multi_submission_result.m_tx_results.contains(wtxid)) {
            ...
            // Already-in-mempool transaction. Check to see if it's still there, as it could have been evicted when LimitMempoolSize() was called.
                                       ...
                                       int64_t accept_time, bool bypass_limits, bool test_accept)
{
    ...
    if (result.m_result_type != MempoolAcceptResult::ResultType::VALID) {
        // Remove coins that were not present in the coins cache before AcceptSingleTransaction—this prevents memory DoS from a large number of invalid txs attempting to overrun the in-memory coins cache (CCoinsViewCache::cacheCoins).
...
{
    return GetTKNCBlockSubsidy(nHeight);
}
    ...
    : m_signature_cache{signature_cache_bytes}
{
    ...
    // We want the nonce to be 64 bytes long to force the hasher to process this chunk, which makes later hash computations more efficient. We just write our 32-byte entropy twice to fill the 64 bytes.
                       ...
                       std::vector<CScriptCheck>* pvChecks)
{
    ...
    // First check if script executions have been cached with the same flags. Assumes that the inputs are correct (the tx hash in tx's prevouts properly commits to the scriptPubKey in the inputs view of that transaction).
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        // We very carefully only pass in things to CScriptCheck which are clearly committed to by tx' witness hash. This provides a sanity check that our caching is not introducing consensus failures through additional data in, eg, the coins being spent being checked as a part of CScriptCheck.
        ...
        if (pvChecks) {
            ...
            // Tx failures never trigger disconnections/bans. This prevents network splits triggered either by non-consensus relay policies (non-standard DER encodings, non-null dummy arguments) or by new consensus rules introduced in soft forks.
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        // Missing undo metadata (height and coinbase). Older versions included this information only in undo records for the last spend of a transaction's outputs. This implies it must be present for some other output of the same tx.
    ...
    // If the coin already exists as an unspent coin in the cache, then the possible_overwrite parameter to AddCoin must be set to true. We've already checked whether an unspent coin exists above using HaveCoin, so we don't need to guess. When fClean is false, an unspent coin already existed and it's an overwrite.
...
script_verify_flags GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // BIP16 didn't become active until Apr 1 2012 (on mainnet, and retroactively applied to testnet). However, only one historical block violated the P2SH rules (on both mainnet and testnet). Similarly, only one historical block violated the TAPROOT rules on mainnet. For simplicity, always leave P2SH+WITNESS+TAPROOT on except for the two violating blocks.
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Check it again in case a previous version let a bad block in. NOTE: We don't currently (re-)invoke ContextualCheckBlock() or ContextualCheckBlockHeader() here. If we add a new consensus rule enforced in one of those, we may have let in a block violating it prior to software update, and we would NOT be enforcing the rule here. Fully solving upgrade after a consensus rule change is potentially tricky and issue-specific (see NeedsRedownload() for the BIP 141 approach).
    ...
    if (!CheckBlock(block, state, params.GetConsensus(), !fJustCheck, !fJustCheck)) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
            // We don't write down blocks to disk if they may have been corrupted, so this should be impossible unless we're having hardware problems.
    ...
    if (m_chainman.AssumedValidBlock().IsNull()) {
        ...
        // We've been configured with the hash of a block that has been externally verified to have a valid history. A suitable default value is included with the software and updated from time to time. Since validity relative to a piece of software is an objective fact, these defaults can be easily reviewed. This setting doesn't force selection of any particular chain but makes validating some faster by effectively caching part of the verification.
        ...
        if (it == m_blockman.m_block_index.end()) {
            ...
            // Block is a member of the assumed verified chain and an ancestor of the best header. Script verification is skipped when connecting blocks under the assumevalid block. Assuming the assumevalid block is valid is safe because block merkle hashes are still computed and checked. Of course, if an assumed valid block is invalid due to false scriptSigs, this optimization would allow an invalid chain to be accepted. The equivalent time check discourages hash power from extorting the network via DoS into accepting an invalid block by telling users they must manually set assumevalid. Requiring a software change or burying the invalid block, regardless of the setting, makes it hard to hide the implication of the demand. This also avoids release candidates that hardly do any signature verification in testing without artificially setting the default assumed verified block further back. The test against minimum chain work prevents skipping when denied access to any chain at least as good as the expected chain.
    ...
    // Do not allow blocks that contain transactions which 'overwrite' older transactions, unless those are already completely spent. If such overwrites are allowed, coinbases and transactions depending upon those can be duplicated to remove the ability to spend the first instance -- even after being sent to another address. See BIP30, CVE-2012-1909, and https://r6.ca/blog/20120206T005236Z.html for more information. This rule was originally applied to all blocks with a timestamp after March 15, 2012, 0:00 UTC. Now that the whole chain is irreversibly beyond that time it is applied to all blocks except the two in the chain that violate it. This prevents exploiting the issue against nodes during their initial block download.
    ...
    // BIP34 requires block X's coinbase scriptSig start with CScriptNum of X (indicated height). The logic of no longer requiring BIP30 once BIP34 activates is flawed for a block X before BIP34 height 227,931 with indicated height Y > X: the coinbase for X would also be valid for Y (BIP30 violation). Exhaustive search of all mainnet coinbases before BIP34 height with indicated height > block height reveals many occurrences; the 3 lowest indicated heights are 209,921, 490,897, and 1,983,702—first opportunity for BIP30 to be violated.
    ...
    // The final case is block 176,684 with indicated height 490,897. The issue was not discovered until about 2 weeks before block 490,897, so there was little opportunity to address it other than careful analysis. Block 490,897 was mined with a different coinbase than block 176,684, but even if it hadn't been or is remined on an alternate fork with a duplicate coinbase, we would still not hit a BIP30 violation. The coinbase for 176,684 is spent in block 185,956 in tx d4f7fbbf92f4a3014a230b2dc70b8058d02eb36ac06b4a0736d9d60eaa9e8781, which can't be duplicated because it also spends coinbase 0328dd85c331237f18e781d692c92de57649529bd5edf1d01036daea32ffde29. That coinbase has indicated height over 4.2 billion, and wouldn't be duplicatable until that height (impossible to create such a chain now). We may still consider a future soft fork retroactively preventing block 490,897 from creating a duplicate coinbase. The two historical BIP30 violations provide a confusing UTXO edge case—simpler not to have another.
    ...
    // TODO: Remove BIP30 checking from block height 1,983,702 on, once we have a consensus change that ensures coinbases at those heights cannot duplicate earlier coinbases.
    ...
    // Precomputed transaction data pointers must not be invalidated until after `control` has run the script checks (potentially in multiple threads). Preallocate the vector size so a new allocation doesn't invalidate pointers into the vector, and keep txsdata in scope for as long as `control`.
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // Check that transaction is BIP68 final. BIP68 lock checks (as opposed to nLockTime checks) must be in ConnectBlock because they require the UTXO set.
        ...
        // GetTransactionSigOpCost counts 3 types of sigops: legacy (always), p2sh (when P2SH enabled in flags and excludes coinbase), witness (when witness enabled in flags and excludes coinbase).
    ...
    // TKNC: Consensus-enforced 90/10 block reward split (team wallet)
    if (pindex->nHeight > 0 && state.IsValid()) {
        ...
        if (coinbase.vout.size() < 2) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-teamwallet",
                          "coinbase must have at least 2 outputs (miner + team wallet)");
        }
        CTxDestination teamDest = DecodeDestination(TKNC_TEAM_WALLET_ADDRESS);
        CScript expectedTeamScript = GetScriptForDestination(teamDest);
        if (coinbase.vout[1].scriptPubKey != expectedTeamScript) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-teamwallet",
                          "coinbase vout[1] does not match team wallet address");
        }
    ...
    try {
    {
        ...
        if (should_write) {
            ...
            }
            {
                ...
                // First make sure all block and undo data is flushed to disk. TODO: handle return error, or add detailed comment why it is safe to not return an error upon failure.
            ...
            if (!CoinsTip().GetBestBlock().IsNull()) {
                // Coin structures on disk are ~48 bytes; pushing a new one can be written twice (log + tables). Most overwrite or delete existing entries, so this overestimates; use a conservative safety factor of 2.
...
/** Disconnect m_chain's tip. After calling, mempool is inconsistent with disconnected-block txs in disconnectpool; call MaybeUpdateMempoolForReorg with cs_main held to restore. If disconnectpool is nullptr, no disconnected txs are added (caller is responsible for mempool consistency). */
bool Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    if (disconnectpool && m_mempool) {
        // Save txs to re-add to mempool at end of reorg; evict entries that exceed memory limits, removing them and their descendants.
...
/** Connect a new block to m_chain. block_to_connect is either nullptr or a CBlock pointer for pindexNew (to bypass disk load). The block is added to connected_blocks on success. */
    ...
    DisconnectedBlockTransactions& disconnectpool)
{
    ...
    // See if this chainstate has reached a target block and can be used to validate an assumeutxo snapshot. If it can, hashing the UTXO database will be slow, and cs_main could remain locked here for several minutes. If the snapshot is validated, the UTXO hash is saved to this->m_target_utxohash, causing HistoricalChainstate() to return null and this chainstate to no longer be used. ActivateBestChain() will also stop connecting blocks to this chainstate because this->ReachedTarget() will be true and this->setBlockIndexCandidates will not have additional blocks.
...
CBlockIndex* Chainstate::FindMostWorkChain()
{
    ...
    do {
        ...
        for (CBlockIndex *pindexTest = pindexNew; pindexTest && !m_chain.Contains(*pindexTest); pindexTest = pindexTest->pprev) {
            ...
            // Pruned nodes may have entries in setBlockIndexCandidates for which block files have been deleted. Remove those as candidates for the most work chain if we come across them; we can't switch to a chain unless we have all the non-active-chain parent blocks.
            ...
            if (fFailedChain || fMissingData) {
                ...
                for (CBlockIndex *pindexFailed = pindexNew; pindexFailed != pindexTest; pindexFailed = pindexFailed->pprev) {
                    if (fMissingData && !fFailedChain) {
                        // If we're missing data and not a descendant of an invalid block, add back to m_blocks_unlinked, so that if the block arrives in the future we can try adding to setBlockIndexCandidates again.
...
/** Try to make some progress towards making index_most_work the active block. pblock is either nullptr or a CBlock pointer for index_most_work. @returns true unless a system error occurred */
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex& index_most_work, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, std::vector<ConnectedBlock>& connected_blocks)
{
    ...
    while (m_chain.Tip() && m_chain.Tip() != pindexFork) {
        if (!DisconnectTip(state, &disconnectpool)) {
            ...
            // If we're unable to disconnect a block during normal operation, then that is a failure of our local system -- we should abort rather than stay on a less work chain.
        ...
        for (CBlockIndex* pindexConnect : vpindexToConnect | std::views::reverse) {
            if (!ConnectTip(state, pindexConnect, pindexConnect == &index_most_work ? pblock : std::shared_ptr<const CBlock>(), connected_blocks, disconnectpool)) {
                if (state.IsInvalid()) {
                    ...
                    // A system error occurred (disk space, database error, ...). Make the mempool consistent with the current tip, just in case any observers try to use it before shutdown.
                    MaybeUpdateMempoolForReorg(disconnectpool, false);
                    return false;
                }
            } else {
    ...
    if (fBlocksDisconnected) {
        // If any blocks were disconnected, disconnectpool may be non empty. Add any disconnected transactions back to the mempool.
...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    do {
        // Block until the validation queue drains. This should largely never happen in normal operation, however may happen during reindex, causing memory blowup if we run too far ahead. Note that if a validationinterface callback ends up calling ActivateBestChain this may lead to a deadlock! We should probably have a DEBUG_LOCKORDER test for this in the future.
...

        {
            ...
            do {
                ...
                // BlockConnected signals must be sent for the original role; in case snapshot validation is completed during ActivateBestChainStep, the result of GetRole() changes from BACKGROUND to NORMAL.
            ...
            if (this == &m_chainman.ActiveChainstate() && pindexFork != pindexNewTip) {
                ...
                {
                    // Just breaking and returning success for now. This could be changed to bubble up the kernel::Interrupted value to the caller so the caller could distinguish between completed and interrupted operations.
                    break;
                }
            }
        ...
        if (reached_target) {
            // Chainstate has reached the target block, so exit. Restart indexes so they can resync and index new blocks after the target block. This cannot be done while holding cs_main (within MaybeValidateSnapshot) or a cs_main deadlock will occur.
        ...
        // We check interrupt only after giving ActivateBestChainStep a chance to run once so that we never interrupt before connecting the genesis block during LoadChainTip(). Previously this caused an assert() failure during interrupt in such cases as the UTXO DB flushing checks that the best block hash is non-null.
...
bool Chainstate::InvalidateBlock(BlockValidationState& state, CBlockIndex* const pindex)
{
    ...
    // We do not allow ActivateBestChain() to run while InvalidateBlock() is running, as that could cause the tip to change while we disconnect blocks.
    LOCK(m_chainstate_mutex);

    // We'll be acquiring and releasing cs_main below, to allow the validation callbacks to run. However, we should keep the block index in a consistent state as we disconnect blocks -- in particular we need to add equal-work blocks to setBlockIndexCandidates as we disconnect. To avoid walking the block index repeatedly in search of candidates, build a map once so that we can look up candidate blocks by chain work as we go.
...

    {
        ...
        for (auto& entry : m_blockman.m_block_index) {
            ...
            // We don't need to put anything in our active chain into the multimap, because those candidates will be found and considered as we disconnect. Instead, consider only non-active-chain blocks that score at least as good with CBlockIndexWorkComparator as the new tip.
    ...
    while (true) {
        ...
        // DisconnectTip will add transactions to disconnectpool. Adjust the mempool to be consistent with the new tip, adding transactions back to the mempool if disconnecting was successful, and we're not doing a very deep invalidation (in which case keeping the mempool up to date is probably futile anyway).
        MaybeUpdateMempoolForReorg(disconnectpool, /* fAddToMempool = */ (++disconnected <= 10) && ret);
        if (!ret) return false;
        CBlockIndex* new_tip{m_chain.Tip()};
        assert(disconnected_tip->pprev == new_tip);

        // We immediately mark the disconnected blocks as invalid. This prevents a case where pruned nodes may fail to invalidateblock and be left unable to start as they have no tip candidates (as there are no blocks that meet the "have data and are not invalid per nStatus" criteria for inclusion in setBlockIndexCandidates).
        ...
        // Mark out-of-chain descendants of the invalidated block as invalid. Add any equal or more work headers that are not invalidated to setBlockIndexCandidates. Recalculate m_best_header if it became invalid.
...

    {
        ...
        // If any new blocks somehow arrived while we were disconnecting (above), then the pre-calculation of what should go into setBlockIndexCandidates may have missed entries. This would technically be an inconsistency in the block index, but if we clean it up here, this should be an essentially unobservable error. Loop back over all block index entries and add any missing entries to setBlockIndexCandidates.
    ...
    if (pindex_was_in_chain) {
        // Ignoring return value for now, this could be changed to bubble up kernel::Interrupted value to the caller so the caller could distinguish between completed and interrupted operations. It might also make sense for the blockTip notification to have an enum parameter indicating the source of the tip change so hooks can distinguish user-initiated invalidateblock changes from other changes.
...
void Chainstate::TryAddBlockIndexCandidate(CBlockIndex* pindex)
{
    ...
    // Do not continue building a chainstate that is based on an invalid snapshot. This is a belt-and-suspenders type of check because if an invalid snapshot is loaded, the node will shut down to force a manual intervention. But it is good to handle this case correctly regardless.
...
void ChainstateManager::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos)
{
    ...
    if (pindexNew->pprev == nullptr || pindexNew->pprev->HaveNumChainTxs()) {
        ...
        while (!queue.empty()) {
            ...
            queue.pop_front();
            // Before setting m_chain_tx_count, assert that it is 0 or already set to the correct value. This assert will fail after receiving the assumeutxo snapshot block if assumeutxo snapshot metadata has an incorrect hardcoded AssumeutxoData::m_chain_tx_count value.
...
{
    if (fCheckPOW) {
        // Use CheckProofOfWork(header) which validates nBits, nTime, and hash <= target.
...
static bool CheckMerkleRoot(const CBlock& block, BlockValidationState& state)
{
    ...
    // Check for merkle tree malleability (CVE-2012-2459): repeating sequences of transactions in a block without affecting the merkle root of a block, while still invalidating it.
    if (mutated) {
        return state.Invalid(
            /*result=*/BlockValidationResult::BLOCK_MUTATED,
...
/** CheckWitnessMalleation performs checks for block malleation with regard to its witnesses. Note: If the witness commitment is expected (i.e. expect_witness_commitment=true), then the block is required to have at least one transaction and the first transaction needs to have at least one input. */
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        // Consider the block mutated if any transaction is 64 bytes in size (see 3.1 in "Weaknesses in TKNC Merkle Root Construction": https://lists.linuxfoundation.org/pipermail/tknc-dev/attachments/20190225/a27d8837/attachment-0001.pdf). Note: This is not a consensus change as this only applies to blocks that don't have a coinbase transaction and would therefore already be invalid.
        return std::any_of(block.vtx.begin(), block.vtx.end(),
                           [](auto& tx) { return GetSerializeSize(TX_NO_WITNESS(tx)) == 64; });
    } else {
        // Theoretically it is still possible for a block with a 64 byte coinbase transaction to be mutated but we neglect that possibility here as it requires at least 224 bits of work.
    }
 ...
 *  grow our in-memory block index. See https://tkncore.org/en/2024/07/03/disclose-header-spam.
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // TKNC uses exact difficulty match per-block with no 4x clamp.
    ...
    // blocks to prevent timewarp attacks (see https://github.com/tknc/tknc/pull/15482).
    if (consensusParams.enforce_BIP94) {
        // TKNC DAA retargets per-block, so timewarp check must also be per-block.
        if (block.GetBlockTime() < pindexPrev->GetBlockTime() - MAX_TIMEWARP) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "time-timewarp-attack", "block's timestamp too early - timewarp protection");
        }
    }
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // Validation for witness commitments. * We compute the witness hash (which is the hash including witnesses) of all the block's transactions, except the coinbase (where 0x0000....0000 is used instead). * The coinbase scriptWitness is a stack of a single 32-byte vector, containing a witness reserved value (unconstrained). * We build a merkle tree with all those witness hashes as leaves (similar to the hashMerkleRoot in the block header). * There must be at least one output whose scriptPubKey is a single 36-byte push, the first 4 bytes of which are {0xaa, 0x21, 0xa9, 0xed}, and the following 32 bytes are SHA256^2(witness root, witness reserved value). In case there are multiple, the last one is used.
...
void ChainstateManager::ReportHeadersPresync(int64_t height, int64_t timestamp)
{
    ...
    {
        LOCK(GetMutex());
        // Don't report headers presync progress if we already have a post-minchainwork header chain. This means we lose reporting for potentially legitimate, but unlikely, deep reorgs, but prevent attackers that spam low-work headers from filling our logs.
        if (m_best_header->nChainWork >= UintToArith256(GetConsensus().nMinimumChainWork)) return;
        // Rate limit headers presync updates to 4 per second, as these are not subject to DoS protection.
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    // TODO: Decouple this function from the block download logic by removing fRequested. This requires some new chain data structure to efficiently look up if a block is in a chain leading to a candidate for best tip, despite not being such a candidate itself. Note that this would break the getblockfrompeer RPC

    // TODO: deal better with return value and error conditions for duplicate and unrequested blocks.
    ...
    if (!fRequested) {  // If we didn't ask for it:
        ...
        // Protect against DoS attacks from low-work chains. If our tip is behind, a peer could try to send us low-work blocks on a fake chain that we would never request; don't process these.
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // CheckBlock() does not support multi-threaded block validation because CBlock::fChecked can cause data race. Therefore, the following critical section must include the CheckBlock() call as well.
        LOCK(cs_main);

        // Skipping AcceptBlock() for CheckBlock() failures means that we will never mark a block as invalid if CheckBlock() fails. This is protective against consensus failure if there are any unknown forms of block malleability that cause CheckBlock() to fail; see e.g. CVE-2012-2459 and https://lists.linuxfoundation.org/pipermail/tknc-dev/2019-February/016697.html. Because CheckBlock() is not very expensive, the anti-DoS benefits of caching failure (of a definitely-invalid block) are not substantial.
...
{
    // Lock must be held throughout this function for two reasons: 1. We don't want the tip to change during several of the validation steps 2. To prevent a CheckBlock() race condition for fChecked, see ProcessNewBlock()
    ...
    if (!CheckBlock(block, state, chainstate.m_chainman.GetConsensus(), /*fCheckPow=*/check_pow, /*fCheckMerkleRoot=*/check_merkle_root)) {
        // This should never happen, but belt-and-suspenders don't approve the block if it does.
    ...
    // We don't want ConnectBlock to update the actual chainstate, so create a cache on top of it, along with a dummy block index.
...
void Chainstate::PopulateBlockIndexCandidates()
{
    ...
    for (CBlockIndex* pindex : m_blockman.GetAllBlockIndices()) {
        // With assumeutxo, the snapshot block is a candidate for the tip, but it may not have BLOCK_VALID_TRANSACTIONS (e.g. if we haven't yet downloaded the block), so we special-case it here.
...
bool Chainstate::LoadGenesisBlock()
{
    ...
    // Check whether we're already initialized by checking for genesis in m_blockman.m_block_index. Note that we can't use m_chain here, since it is set based on the coins db, not the block index db, which is the only thing loaded at this point.
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // Activate the genesis block so normal node progress can continue. During first -reindex, this will only connect Genesis since ActivateBestChain only connects blocks which are in the block tree db, which only contains blocks whose parents are in it. But do this only if genesis isn't activated yet, to avoid connecting many blocks without assumevalid in the case of a continuation of a reindex that was interrupted by the user.
                ...
                if (m_blockman.IsPruneMode() && m_blockman.m_blockfiles_indexed && pblock) {
                    // Must update the tip for pruning to work while importing with -loadblock. This is a tradeoff to conserve disk space at the expense of time spent updating the tip to be able to prune. Otherwise, ActivateBestChain won't be called by the import process until after all of the block files are loaded. ActivateBestChain can be called by concurrent network message processing. but, that is not reliable for the purpose of pruning while importing.
...
void ChainstateManager::CheckBlockIndex() const
{
    ...
    // During a reindex, we read the genesis block and call CheckBlockIndex before ActivateBestChain, so we have the genesis block in m_blockman.m_block_index but no active chain. (A few of the tests when iterating the block tree require that m_chain has been initialized.)
    if (ActiveChain().Height() < 0) {
        assert(m_blockman.m_block_index.size() <= 1);
        return;
    }

    // Build forward-pointing data structure for the entire block tree. For performance reasons, indexes of the best header chain are stored in a vector (within CChain). All remaining blocks are stored in a multimap. The best header chain can differ from the active chain: E.g. its entries may belong to blocks that are not yet validated.
    ...
    assert(pindex);
    // Iterate over the entire block tree, using depth-first search. Along the way, remember whether there are blocks on the path from genesis block being explored which are the first to have certain properties.
    size_t nNodes = 0;
    int nHeight = 0;
    ...
    // After checking an assumeutxo snapshot block, reset pindexFirst pointers to earlier blocks that have not been downloaded or validated yet, so checks for later blocks can assume the earlier blocks were validated and be stricter, testing for more requirements.
    ...
    while (pindex != nullptr) {
        ...
        if (pindex->pprev != nullptr && pindex->nChainWork < pindex->pprev->nChainWork) {
            LogError("WARNING: nChainWork inconsistency at height %d: current=%s, parent=%s. Auto-correcting.\n",
                     pindex->nHeight, pindex->nChainWork.GetHex(), pindex->pprev->nChainWork.GetHex());
            const_cast<CBlockIndex*>(pindex)->nChainWork = pindex->pprev->nChainWork + 1;
        }
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Datadir should be removed from filesystem; otherwise initialization may detect it on subsequent statups and get confused. If the base_blockhash_path removal above fails in the case of snapshot chainstates, this will return false since leveldb won't remove a non-empty directory.
        ...
        bool in_memory)
{
    ...
    auto cleanup_bad_snapshot = [&](bilingual_str reason) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        ...
        if (auto snapshot_datadir = node::FindAssumeutxoChainstateDir(m_options.datadir)) {
            // We have to destruct leveldb::DB in order to release the db lock, otherwise DestroyDB() (in DeleteCoinsDBFromDisk()) will fail. See `leveldb::~DBImpl()`. Destructing the chainstate (and so resetting the coinsviews object) does this.
    ...
    // Do a final check to ensure that the snapshot chainstate is actually a more work chain than the active chainstate; a user could have loaded a snapshot very late in the IBD process, and we wouldn't want to load a useless chainstate.
    ...
    const SnapshotMetadata& metadata)
{
    ...
    // This work comparison is a duplicate check with the one performed later in ActivateSnapshot(), but is done so that we avoid doing the long work of staging a snapshot that isn't actually usable.
    ...
    while (coins_left > 0) {
        try {
            ...
            for (size_t i = 0; i < coins_per_txid; i++) {
                ...
                // Batch write and flush (if we need to) every so often. If our average Coin size is roughly 41 bytes, checking every 120,000 coins means <5MB of memory imprecision.
                if (coins_processed % 120000 == 0) {
                    ...
                    if (snapshot_cache_state >= CoinsCacheSizeState::CRITICAL) {
                        // This is a hack - we don't know what the actual best block is, but that doesn't matter for the purposes of flushing the cache here. We'll set this to its correct value (`base_blockhash`) below after the coins are loaded.
    ...
    // Important that we set this. This and the coins_cache accesses above are sort of a layer violation, but either we reach into the innards of CCoinsViewCache here or we have to invert some of the Chainstate to embed them in a snapshot-activation-specific CCoinsViewCache bulk load method.
    ...
    // Don't make any modifications to the genesis block since it shouldn't be necessary, and since the genesis block doesn't have normal flags like BLOCK_VALID_SCRIPTS set.
    ...
    for (int i = AFTER_GENESIS_START; i <= snapshot_chainstate.m_chain.Height(); ++i) {
        ...
        // Changes to the block index will be flushed to disk after this call returns in `ActivateSnapshot()`, when `MaybeRebalanceCaches()` is called, since we've added a snapshot chainstate and therefore will have to downsize the IBD chainstate, which will result in a call to `FlushStateToDisk(FORCE_FLUSH)`.
    }
...
// Currently, this function holds cs_main for its duration, which could be for multiple minutes due to the ComputeUTXOStats call. Holding cs_main used to be necessary (before d43a1f1a2fa3) to avoid advancing validated_cs farther than its target block. Now it should be possible to avoid this, but simply releasing cs_main here would not be possible because this function is invoked by ConnectTip within ActivateBestChain. Eventually (TODO) it would be better to call this function outside of ActivateBestChain, on a separate thread that should not require cs_main to hash, because the UTXO set is only hashed after the historical chainstate reaches its target block and is no longer changing.
SnapshotCompletionResult ChainstateManager::MaybeValidateSnapshot(Chainstate& validated_cs, Chainstate& unvalidated_cs)
{
    ...
    if (!validated_cs_stats) {
        ...
        // While this isn't a problem with the snapshot per se, this condition prevents us from validating the snapshot, so we should shut down and let the user handle the issue manually.
        handle_invalid_snapshot();
        return SnapshotCompletionResult::STATS_FAILED;
    }

    // Compare the validated chainstate's UTXO set hash against the hard-coded assumeutxo hash we expect. TODO: For belt-and-suspenders, we could cache the UTXO set hash for the snapshot when it's loaded in its chainstate's leveldb. We could then reference that here for an additional check.
...
void ChainstateManager::MaybeRebalanceCaches()
{
    ...
    if (!historical_cs && !current_cs.m_from_snapshot_blockhash) {
        ...
        // If both chainstates exist, determine who needs more cache based on IBD status. Note: shrink caches first so that we don't inadvertently overwhelm available memory.
...
util::Result<void> Chainstate::InvalidateCoinsDBOnDisk()
{
    ...
    // The invalid storage directory is simply moved and not deleted because we may want to do forensics later during issue investigation. The user is instructed accordingly in MaybeValidateSnapshot().
    try {
...
std::pair<int, int> Chainstate::GetPruneRange(int last_height_can_prune) const
{
    ...
    if (m_from_snapshot_blockhash && m_assumeutxo != Assumeutxo::VALIDATED) {
        // Only prune blocks _after_ the snapshot if this is a snapshot chain that has not been fully validated yet. The earlier blocks need to be kept to validate the snapshot
...
{
    // We can't hold cs_main during ActivateBestChain even though we're accessing the chainman unique_ptrs since ABC requires us not to be holding cs_main, so retrieve the relevant pointers before the ABC call.
    AssertLockNotHeld(cs_main);

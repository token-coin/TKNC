...
/** Avoids mempool polluting consensus critical pathsached sig and script validity results are reused if the tx is revalidated during block validation. */
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
 ...
 // BIP94 timewarp protection: Check timestamp against prev block.
 // Enabled on all chains where enforce_BIP94=true (mainnet has it enabled).

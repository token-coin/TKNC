...
void CCoinsViewCache::AddCoin(const COutPoint &outpoint, Coin&& coin, bool possible_overwrite) {
    ...
    if (!possible_overwrite) {
        ...
        // Spent+DIRTY coin: spentness not yet flushed to parent, so can't mark FRESH (else spend-before-flush loses it).
        // Re-add happens on re-org. If coin absent or spent-but-not-DIRTY, it can be marked FRESH.
    ...
    for (auto it{cursor.Begin()}; it != cursor.End(); it = cursor.NextAndMaybeErase(*it)) {
        ...
        if (inserted) {
            if (it->second.IsFresh() && it->second.coin.IsSpent()) {
                ...
                // Mark FRESH in parent only if FRESH in child (else may exist in grandparent from prior flush).
                if (it->second.IsFresh()) CCoinsCacheEntry::SetFresh(*itUs, m_sentinel);
            }
        } else {
            // Found the entry in the parent cache
            if (it->second.IsFresh() && !itUs->second.coin.IsSpent()) {
                // FRESH in child but exists in parent: FRESH flag misapplied — logic error in caller.
            ...
            if (itUs->second.IsFresh() && it->second.coin.IsSpent()) {
                ...
                // Not safe to mark FRESH in parent: if it existed and was spent there, FRESH would skip flushing spentness to grandparent.
            }
        }
    }
...
static ReturnType ExecuteBackedWrapper(Func func, const std::vector<std::function<void()>>& err_callbacks)
{
    try {
        ...
        // Returning false would be misread as 'entry not found'; abort instead (writes must be atomic).
        std::abort();
    }
}

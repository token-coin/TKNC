...
{
    // Strategy: try both extended (witness) and legacy serialization; prefer extended on conflict.
    // try_no_witness/try_witness restrict modes; CheckTxScriptsSanity breaks ties (extended wins if both/neither pass).
...
/** Create asm string for CScript. @param script CScript to convert. @param fAttemptSighashDecode decode sighash types (only for suspected sigs, not scriptPubKeys). */
        ...
        if (0 <= opcode && opcode <= OP_PUSHDATA4) {
            if (vch.size() <= static_cast<std::vector<unsigned char>::size_type>(4)) {
                ...
                if (fAttemptSighashDecode && !script.IsUnspendable()) {
                    ...
                    // Only decode sighash from signature-like data in scriptSig; pubkeys in Pubkey/Multisig won't match (IsCompressedOrUncompressedPubKey vs CheckSignatureEncoding incongruous).

...
// ============================================================
// Legacy API implementation — delegates to BackendRouter
// ============================================================
    ...
    const std::vector<InferenceMessage>& messages)
{
    ...
    // Convert legacy InferenceMessage → ComputeRequest
    ...
    // Convert ComputeResponse → legacy InferenceResult (backward compatible)
...
void InferenceEngine::InitP2PRemoteRouting(CConnman* connman)
{
    ...
    // Bind CConnman callbacks (bridge: RemoteBackend → CConnman message layer)
    // SendCallback: sends APIREQ to target peer via CConnman P2P network.
    // Uses promise/future for async response — matches net_processing.cpp L5403-5435 pattern.
        ...
        int64_t& out_cost) -> bool
    {
        ...
        // Response received — extract from fulfilled promise
    ...
    // Bind RefreshCallback — lazy peer discovery at inference time
    remote->SetRefreshCallback([connman]() {
        InferenceEngine::ScanAndRegisterPeers(connman);
    });

    // Enable RemoteBackend — it's now fully wired
...
// ============================================================
// HandleAPIResponse — Bridge: APIRESP message → RemoteBackend promise
// ============================================================
    ...
    int64_t cost)
{
    ...
    std::shared_ptr<std::promise<P2PPendingResponse>> promise;
    {
        ...
        if (it == s_pending_responses.end()) {
            // Not our request — belongs to net_processing.cpp's own pending map
            return;
        }
        promise = it->second;
    ...
    // Fulfill the promise — unblocks SendCallback's future.get()

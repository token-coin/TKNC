...
std::vector<uint8_t> P2PLLMHandshakeReq::Serialize() const {
 ...
 int64_t net_price = HtoN64(static_cast<uint64_t>(client_tokens_per_tknc));
...
bool P2PLLMHandshakeReq::Deserialize(const std::vector<uint8_t>& data) {
 ...
 client_tokens_per_tknc = static_cast<int64_t>(NtoH64(net_price));
 return true;
}
...
// verified_tokens_per_tknc
int64_t net_vp = HtoN64(static_cast<uint64_t>(verified_tokens_per_tknc));
...
bool P2PLLMHandshakeResp::Deserialize(const std::vector<uint8_t>& data) {
 ...
 verified_tokens_per_tknc = static_cast<int64_t>(NtoH64(net_vp));
 pos += sizeof(uint64_t);
 if (pos + sizeof(int64_t) > data.size()) return false;
 uint64_t net_tp;
...
void P2PLLMPeerHandler::ProcessInferenceRequest(const std::string& peer_id,
 ...
 // SECURITY: Pre-inference billing check — refuse if the requesting peer's
 // API key has insufficient balance. This prevents free inference via P2P.
 // Without this check, any connected peer could get free inference by sending
 // P2P inference requests, bypassing the HTTP gateway's billing entirely.
 if (!req.api_key.empty()) {
 EnsureEscrowForAPIKey(req.api_key);
 std::string balance_error;
 if (!CanAffordInference(req.api_key, balance_error)) {
 LogWarning("[P2P-BILLING] Inference REFUSED for peer=%s, api_key=%s...: %s",
 peer_id.c_str(), req.api_key.substr(0, 8).c_str(), balance_error.c_str());
 P2PLLMInferenceError err;
 err.error_code = 402;
 err.error_message = "Inference refused: " + balance_error;
 SendInferenceError(socket_fd, err);
 return;
 }
 // SECURITY: Miner-side independent payment verification.
 // The miner independently checks if it has received sufficient TKNC from
 // this client. This prevents billing bypass where a malicious client
 // modifies their own node code to skip billing — the miner checks its
 // own records, not trusting the client's escrow state.
 std::string miner_pay_error;
 if (!CheckMinerReceivedPayment(req.api_key, miner_pay_error)) {
 LogWarning("[P2P-MINER-PAY] Inference REFUSED for peer=%s, api_key=%s...: %s",
 peer_id.c_str(), req.api_key.substr(0, 8).c_str(), miner_pay_error.c_str());
 P2PLLMInferenceError err;
 err.error_code = 402;
 err.error_message = "Inference refused: " + miner_pay_error;
 SendInferenceError(socket_fd, err);
 return;
 }
 }
 ...
 // SECURITY: Post-inference billing — deduct tokens from escrow.
 // Without this, P2P inference is free even though the pre-check passed.
 // Do NOT bill/track if the miner returned an error response — the content
 // may contain error markers like [Error:], [Inference unavailable:], etc.
 bool p2p_miner_error = (result.content.empty()
 || result.content.find("[Error:") != std::string::npos
 || result.content.find("[Miner returned") != std::string::npos
 || result.content.find("[Inference unavailable:") != std::string::npos
 || result.content.find("[LLM inference error:") != std::string::npos
 || result.content.find("[FATAL:") != std::string::npos);

 if (!req.api_key.empty() && done.total_tokens > 0 && !p2p_miner_error) {
 // Track served tokens for miner-side independent payment verification
 TrackMinerServedTokens(req.api_key, done.total_tokens);

 BillingReceipt receipt;
 if (CheckAndDeductEscrow(req.api_key, done.total_tokens, receipt)) {
 LogInfo("[P2P-BILLING] Billing SUCCESS: peer=%s, tokens=%d, cost=%s TKNC, consumed=%s TKNC",
 peer_id.c_str(), done.total_tokens,
 FormatMoney(receipt.cost_tknc).c_str(),
 FormatMoney(receipt.consumed_tknc).c_str());
 } else {
 LogWarning("[P2P-BILLING] Billing FAILED: peer=%s, tokens=%d — escrow exhausted/invalid",
 peer_id.c_str(), done.total_tokens);
 }
 } else if (p2p_miner_error) {
 LogWarning("[P2P-BILLING] SKIPPED billing: miner returned error/empty response "
 "(peer=%s, tokens=%d, content_prefix=%.60s)",
 peer_id.c_str(), done.total_tokens, result.content.substr(0, 60).c_str());
 }
...
void P2PLLMPeerHandler::ProcessHandshakeReq(const std::string& peer_id,
 ...
 int socket_fd) {
LogInfo("[HANDSHAKE] Received handshake request from peer=%s, model=%s, client_tokens_per_tknc=%lld",
peer_id.c_str(), req.model_name.c_str(), (long long)req.client_tokens_per_tknc);

P2PLLMHandshakeResp resp;

// Step 1: Get miner's verified token rate from node-side storage
// Use the miner's specific wallet — NOT empty string which returns 0.
int64_t verified_tpt = GetTokensPerTknc(GetMinerWalletAddress());
if (verified_tpt <= 0) verified_tpt = 0; // No rate set — miner must configure -token

resp.verified_tokens_per_tknc = verified_tpt;
resp.tokens_per_tknc = verified_tpt; // Same value, kept for binary compat
 ...
 if (req.client_tokens_per_tknc > 0 && verified_tpt > 0) {
...
std::to_string(verified_tpt) + ". Possible price cheating.";
 SendHandshakeResp(socket_fd, resp);
 return;
 }
 }
...
LogInfo("[HANDSHAKE] Sending response: accepted=%s, tokens_per_tknc=%lld",
resp.accepted ? "true" : "false",
(long long)resp.verified_tokens_per_tknc);
...
LogInfo("[HANDSHAKE] Received handshake response from peer=%s: accepted=%s, tokens_per_tknc=%lld, miner_wallet=%s",
peer_id.c_str(),
resp.accepted ? "true" : "false",
(long long)resp.verified_tokens_per_tknc,

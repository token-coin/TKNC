...
// ============================================================
// Infer — The main P2P routing logic
// ============================================================
ComputeResponse RemoteBackend::Infer(const ComputeRequest& request)
{
    ...
    // Step 4: PREFLIGHT CHECK — force reachability confirmation (BP2 fix)
    ...
    // Step 6: Session already bound by PreflightCheck — skip redundant hole punch/CreateSession
    ...
    // Step 8: Build response — check for error content even when send succeeded
    if (sent_ok && response_content.find("Error:") != 0) {
        // Send OK AND response content is not an error → real success
        result.success = true;
...
// ============================================================
// PreflightCheck — Force reachability confirmation before Infer
// ============================================================
    ...
    const EndpointEntry& target, PeerSession*& out_session)
{
    ...
    // 1. Bind endpoint → session (reuse or create)

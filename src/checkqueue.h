...
#ifndef TKN_CHECKQUEUE_H
#define TKN_CHECKQUEUE_H
...
/** Queue for verifications that have to be performed. The verifications are represented by a type T, which must provide an operator() returning an std::optional<R>. The overall result is std::nullopt if all invocations return std::nullopt, or one of the other results otherwise. One thread (the master) is assumed to push batches of verifications onto the queue, where they are processed by N-1 worker threads. When the master is done adding work, it temporarily joins the worker pool as an N'th worker, until all jobs are done. */
        ...
        do {
            {
                ...
                // Decide how many work units to process now: aim for increasingly smaller batches so all workers finish approximately simultaneously; account for idle jobs which will instantly start helping; never do batches smaller than 1 (duh), or larger than nBatchSize.

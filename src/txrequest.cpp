...
/** The various states a (txhash,peer) pair can be in. CANDIDATE is split into 3 substates (DELAYED, BEST, READY) for more efficient implementation; ByTxHashView sort order relies on the specific order of values in this enum. Expected behaviour: (1) When first announced, state is CANDIDATE_DELAYED until reqtime is reached. (2) Announcements that have reached reqtime but not been requested will be CANDIDATE_READY or CANDIDATE_BEST (no expiration time, remain in state until requested or no longer needed); CANDIDATE_READY promoted to CANDIDATE_BEST when the best one left. (3) When requested, state REQUESTED until expiry reached. (4) On expiry, or peer reply (NOTFOUND or tx), state becomes COMPLETED. */
...
// Definitions for the 3 indexes used in the main data structure. Each index has a By* type to identify it, a By*View data type to represent the view of announcement it is sorted by, and an By*ViewExtractor type to convert an announcement into the By*View type. See https://www.boost.org/doc/libs/1_58_0/libs/multi_index/doc/reference/key_extraction.html#key_extractors for more information about the key extraction concept.

// The ByPeer index is sorted by (peer, state == CANDIDATE_BEST, txhash). Uses: (1) Looking up existing announcements by peer/txhash, by checking both (peer, false, txhash) and (peer, true, txhash). (2) Finding all CANDIDATE_BEST announcements for a given peer in GetRequestable.
struct ByPeer {};
...
// The ByTxHash index is sorted by (txhash, state, priority). Note: priority == 0 whenever state != CANDIDATE_READY. Uses: (1) Deleting all announcements with a given txhash in ForgetTxHash. (2) Finding the best CANDIDATE_READY to convert to CANDIDATE_BEST, when no other CANDIDATE_READY or REQUESTED announcement exists for that txhash. (3) Determining when no more non-COMPLETED announcements for a given txhash exist, so the COMPLETED ones can be deleted.
struct ByTxHash {};
...
private:
    ...
    void PromoteCandidateReady(Iter<ByTxHash> it)
    {
        ...
        // The following code relies on the fact that the ByTxHash is sorted by txhash, then by state (first _DELAYED, then _READY, then _BEST/REQUESTED). Within _READY announcements, the best one (highest priority) comes last. Thus, if an existing _BEST exists for the same txhash that this announcement may be preferred over, it must immediately follow the newly created _READY.
        auto it_next = std::next(it);
        if (it_next == m_index.get<ByTxHash>().end() || it_next->m_gtxid.ToUint256() != it->m_gtxid.ToUint256() ||
            it_next->GetState() == State::COMPLETED) {
            // This is the new best CANDIDATE_READY, and there is no IsSelected() announcement for this txhash already.
    ...
    //! Make the data structure consistent with a given point in time: (1) REQUESTED announcements with expiry <= now are turned into COMPLETED. (2) CANDIDATE_DELAYED announcements with reqtime <= now are turned into CANDIDATE_{READY,BEST}. (3) CANDIDATE_{READY,BEST} announcements with reqtime > now are turned into CANDIDATE_DELAYED.
    void SetTimePoint(std::chrono::microseconds now, std::vector<std::pair<NodeId, GenTxid>>* expired)
    {
        ...
        while (!m_index.empty()) {
            // If time went backwards, we may need to demote CANDIDATE_BEST and CANDIDATE_READY announcements back to CANDIDATE_DELAYED. Unusual edge case, unlikely to matter in production, but makes it much easier to specify and test TxRequestTracker::Impl's behaviour.
...
public:
    ...
    void DisconnectedPeer(NodeId peer)
    {
        ...
        while (it != index.end() && it->m_peer == peer) {
            // Check what to continue with after this iteration. 'it' will be deleted in what follows, so we need to decide what to continue with afterwards. Cases: (1) std::next(it) is end() or belongs to a different peer—last iteration of the loop (set it_next to end()). (2) 'it' is not the only non-COMPLETED announcement for its txhash—will be deleted, no other Announcements modified. Continue with std::next(it) if it belongs to the same peer, but decide this ahead of time ('it' may change position in what follows). (3) 'it' is the only non-COMPLETED announcement for its txhash—will be deleted along with all other announcements for same txhash (may include std::next(it)). However, other than 'it', no announcements for the same peer can be affected (due to (peer, txhash) uniqueness). In other words, the situation where std::next(it) is deleted can only occur if std::next(it) belongs to a different peer but the same txhash as 'it'—covered by the first case, and we'll have set it_next to end().
    ...
    {
        // Bail out if we already have a CANDIDATE_BEST announcement for this (txhash, peer) combination. The case where there is a non-CANDIDATE_BEST announcement already will be caught by the uniqueness property of the ByPeer index when we try to emplace the new object below.
        if (m_index.get<ByPeer>().count(ByPeerView{peer, true, gtxid.ToUint256()})) return;

        // Try creating the announcement with CANDIDATE_DELAYED state (which will fail due to the uniqueness of the ByPeer index if a non-CANDIDATE_BEST announcement already exists with the same txhash and peer). Bail out in that case.
    ...
    void RequestedTx(NodeId peer, const uint256& txhash, std::chrono::microseconds expiry)
    {
        ...
        if (it == m_index.get<ByPeer>().end()) {
            ...
            if (it == m_index.get<ByPeer>().end() || (it->GetState() != State::CANDIDATE_DELAYED &&
                ...
                // There is no CANDIDATE announcement tracked for this peer, so we have nothing to do. Either this txhash wasn't tracked at all (and the caller should have called ReceivedInv), or it was already requested and/or completed for other reasons and this is just a superfluous RequestedTx call.
                return;
            }

            // Look for an existing CANDIDATE_BEST or REQUESTED with the same txhash. We only need to do this if the found announcement had a different state than CANDIDATE_BEST. If it did, invariants guarantee that no other CANDIDATE_BEST or REQUESTED can exist.
            ...
            if (it_old != m_index.get<ByTxHash>().end() && it_old->m_gtxid.ToUint256() == txhash) {
                if (it_old->GetState() == State::CANDIDATE_BEST) {
                    // The data structure's invariants require that there can be at most one CANDIDATE_BEST or one REQUESTED announcement per txhash (but not both simultaneously), so we have to convert any existing CANDIDATE_BEST to another CANDIDATE_* when constructing another REQUESTED. It doesn't matter whether we pick CANDIDATE_READY or _DELAYED here, as SetTimePoint() will correct it at GetRequestable() time. If time only goes forward, it will always be _READY, so pick that to avoid extra work in SetTimePoint().
                    Modify<ByTxHash>(it_old, [](Announcement& ann) { ann.SetState(State::CANDIDATE_READY); });
                } else if (it_old->GetState() == State::REQUESTED) {
                    // As we're no longer waiting for a response to the previous REQUESTED announcement, convert it to COMPLETED. This also helps guarantee progress.

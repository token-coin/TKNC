...
/** How long to delay before querying DNS seeds (longer if addrman has many peers). */
...
enum BindFlags {
    ...
    /** Do not call AddLocal() for special addresses (e.g. Tor inbound) to prevent gossiping. */
...
[[nodiscard]] static std::optional<CService> GetLocal(const CNode& peer)
{
    ...
    int nBestReachability = -1;
    {
        ...
        for (const auto& [local_addr, local_service_info] : mapLocalHost) {
            // Don't advertise privacy-network addresses to other networks and vice versa.
...
{
    // Seed nodes get a random 'last seen time' of 1-2 weeks ago.
...
// Return best local address for peer, or unroutable 0.0.0.0 filled with normal params.
...
std::optional<CService> GetLocalAddrForPeer(CNode& node)
{
    ...
    // If discovery is enabled, sometimes give our peer the address it sees us as.
    FastRandomContext rng;
         ...
         rng.randbits((GetnScore(addrLocal) > LOCAL_MANUAL) ? 3 : 1) == 0))
    {
        if (node.IsInboundConn()) {
            ...
            // For outbound connections, assume only the address (peer can't observe our listening port).
...
bool CConnman::CheckIncomingNonce(uint64_t nonce)
{
    ...
    for (const CNode* pnode : m_nodes) {
        // Omit private broadcast connections to prevent a privacy attack where a peer
        // learns our clearnet address by reusing our nonce.
    ...
    for (auto& target_addr : connect_to) {
        if (target_addr.IsValid()) {
            ...
            if (target_addr.IsI2P() && use_proxy) {
                ...
                // Force a new transient I2P session for private broadcast to avoid leaking our permanent I2P address.
...
int V1Transport::readHeader(std::span<const uint8_t> msg_bytes)
{
    ...
    // Reject oversized messages (previously allowed 32MiB allocation per connection - CVE disclosure).
...
/** List of short messages as defined in BIP324, in order (only implemented types need listing). */
...
void V2Transport::ProcessReceivedMaybeV1Bytes() noexcept
{
    ...
    // Determine v1 vs v2: match against 16-byte v1 prefix; mismatch triggers v2 key sending (BIP324).
...
bool V2Transport::ProcessReceivedKeyBytes() noexcept
{
    ...
    // Detect cross-network peers: if key bytes 4-16 match V1 "version" but bytes 0-4 don't match magic, disconnect.
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    // Max packet contents: 0x00 byte + 12-byte message type + payload.
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        // Decrypt ciphertext (unreachable without hitting the length branch first due to GetMaxBytesToProcess).
...
size_t V2Transport::GetMaxBytesToProcess() noexcept
{
    ...
    switch (m_recv_state) {
    case RecvState::KEY_MAYBE_V1:
        // Limit to v1 prefix length; exceeding v1 header size would prevent feeding bytes to m_v1_fallback.
        Assume(m_recv_buffer.size() <= V1_PREFIX_LEN);
        return V1_PREFIX_LEN - m_recv_buffer.size();
    case RecvState::KEY:
        // Only allow the 64-byte key (garbage follows; terminator requires key exchange first).
        Assume(m_recv_buffer.size() <= EllSwiftPubKey::size());
    ...
    case RecvState::VERSION:
    case RecvState::APP:
        // Process length descriptor first, then ciphertext (encrypted length stays in buffer).
        if (m_recv_buffer.size() < BIP324Cipher::LENGTH_LEN) {
            return BIP324Cipher::LENGTH_LEN - m_recv_buffer.size();
        } else {
            // BIP324Cipher::EXPANSION includes the 3-byte packet length.
...
bool V2Transport::ReceivedBytes(std::span<const uint8_t>& msg_bytes) noexcept
{
    ...
    // Process bytes in a loop: copy to m_recv_buffer, then call ProcessReceived*Bytes by state.
    while (!msg_bytes.empty()) {
        ...
        if (m_recv_buffer.size() + std::min(msg_bytes.size(), max_read) > m_recv_buffer.capacity()) {
            ...
            case RecvState::VERSION:
            case RecvState::APP: {
                // Limit over-allocation to MAX_RESERVE_AHEAD to bound memory waste from attackers.
...
bool V2Transport::SetMessageToSend(CSerializedNetMsg& msg) noexcept
{
    ...
    // Only add messages in READY state with empty buffer (caller responsible for queueing).
...
std::pair<size_t, bool> CConnman::SocketSendData(CNode& node) const
{
    ...
    while (true) {
        if (it != node.vSendMsg.end()) {
            // Move one message to transport (fails if existing message sending or v2 handshake incomplete).
        ...
        // Sanity-check that previous 'more' prediction was correct (used for MSG_MORE flag).
        ...
        if (!data.empty()) {
            ...
            // No socket if disconnected or in test cases; leave data in send queue.
            if (!node.m_sock) {
                break;
            }
...
/** Try to find a connection to evict when the node is full. Protects peers across multiple hard-to-forge characteristics. */
...

    {
        ...
        for (CNode* pnode : nodes_copy)
        {
            if (pnode->fDisconnect)
            {
                ...
                // Defer reconnection (blocking op) to avoid holding up the socket handler thread.
...
Sock::EventsPerSock CConnman::GenerateWaitSockets(std::span<CNode* const> nodes)
{
    ...
    for (CNode* pnode : nodes) {
        ...
        {
            LOCK(pnode->cs_vSend);
            // Sending is possible if bytes are ready now or will be after handing a queued message to transport.
...
void CConnman::SocketHandler()
{
...

    {
        ...
        // Check socket readiness (poll/select); wait briefly if none are ready.
    ...
    for (CNode* pnode : nodes) {
        ...
        if (sendSet) {
            ...
            if (bytes_sent) {
                ...
                // Skip receiving if send queue isn't drained (TCP flow control avoids piling data
                // on slow receivers). Only when send succeeded, to guarantee progress.
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    if (gArgs.GetBoolArg("-forcednsseed", DEFAULT_FORCEDNSSEED)) {
        ...
        // Query all seeds on first run or if peers.dat was deleted.
        seeds_right_now = seeds.size();
    }
    ...
    if (outbound_connection_count < SEED_OUTBOUND_CONNECTION_THRESHOLD || seeds_right_now) {
        // Only query DNS seed when address need is acute. Query a few at a time
        // to prevent eclipse attacks; fall back to all seeds and fixed seeds on failure.
        int found = 0;
        ...
        for (const std::string& seed : seeds) {
            if (seeds_right_now == 0) {
                ...
                if (addrman.get().Size() > 0) {
                    ...
                    while (to_wait.count() > 0) {
                        // Wake up early to check if we have enough peers and can stop this thread.
            ...
            if (HaveNameProxy()) {
                ...
                // Limit IPs per DNS seed to prevent one seed from dominating AddrMan (TCP can return more than UDP's 33).
                ...
                if (!addresses.empty()) {
                    ...
                    // Fallback: ADDR_FETCH the base dns seed domain if subdomain lookup failed.
                    AddAddrFetch(seed);
                }
            }
            --seeds_right_now;
...
// Return count of peers over outbound limit (excludes disconnecting/incomplete-handshake peers).
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect, std::span<const std::string> seed_nodes)
{
    ...
    while (!m_interrupt_net->interrupted()) {
        ...
        if (add_fixed_seeds && !fixed_seed_networks.empty()) {
            // Allow 60s for -dnsseed/-seednode/-addnode to populate addrman before falling back to fixed seeds.
            ...
            if (add_fixed_seeds_now) {
                ...
                // Only add seeds for reachable networks (unreachable networks may become reachable later).
...

        {
            ...
            for (const CNode* pnode : m_nodes) {
                ...
                switch (pnode->m_conn_type) {
                    // Inbound connections are excluded (attackers could spam them to block outbound slots).
                    case ConnectionType::INBOUND:
                    ...
                    case ConnectionType::BLOCK_RELAY:
                        ...
                        if (address.IsTor() || address.IsI2P() || address.IsCJDNS()) {
                            // Privacy networks have random addrman-groups, so netgroup diversity is not enforced.
        ...
        // Determine connection type by priority: anchor BLOCK_RELAY, then OUTBOUND_FULL_RELAY,
        // then BLOCK_RELAY, then extra outbound on stale tip, then extra block-relay/feeler.
        if (!m_anchors.empty() && (nOutboundBlockRelay < m_max_outbound_block_relay)) {
            ...
            // Periodically connect to a peer to sync headers then disconnect if nothing new is learned.
            // Makes eclipse attacks harder; may evict next-youngest block-relay peer (see net_processing.cpp).
            ...
            // Get at least one outbound peer per reachable network; protect them during eviction.
            // Skipped if -maxconnections is too low (would conflict with protected peers).
        ...
        while (!m_interrupt_net->interrupted()) {
            ...
            // Stop after 100 tries; let outer loop run again (sleeps, adds seeds, recalculates networks).
            nTries++;
            if (nTries > 100)
                break;
            ...
            if (fFeeler) {
                ...
                if (!addr.IsValid()) {
                    ...
                    // If already connected to the collision address, mark it Good() to avoid evicting a connected peer.
                    addrman.get().Good(addr);
                    // Select a new table address for our feeler instead.
                    std::tie(addr, addr_last_try) = addrman.get().Select(true, reachable_nets);
                }
            } else {
                // If preferred_net is set, pick from that network (net_processing will evict a peer from another network).
            ...
            // Non-feelers need all desired services; feelers only need a full node (SPV clients lack address DBs).
            ...
            // Skip addnode peers to preserve outbound slots and addnode protections.
        ...
        if (addrConnect.IsValid()) {
            ...
            // Record addrman failures only when node has enough diverse outbound peers (not when offline).
...
void CConnman::ThreadMessageHandler()
{
    ...
    while (!flagInterruptMsgProc)
    {
        ...
        {
            // Randomize peer processing order to prevent attacks exploiting consecutive m_nodes entries.
...
bool CConnman::InitBinds(const Options& options)
{
    ...
    if (options.bind_on_any) {
        // IPv6 "::" bind errors are non-fatal (host OS may lack IPv6 support; user didn't explicitly request it).
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    // TKNC: Start NAT traversal (member variable, lifetime follows CConnman)
    if (fListen) {
        uint16_t port = static_cast<uint16_t>(GetListenPort());

        m_nat_traversal = std::make_unique<NATTraversal>();

        LogInfo("TKNC: Starting NAT traversal on port %d", port);

        if (m_nat_traversal->TryUPnP(port)) {
            LogInfo("TKNC: UPnP port mapping successful");
        } else if (m_nat_traversal->TryNATPMP(port)) {
            LogInfo("TKNC: NAT-PMP port mapping successful");
        } else {
            LogInfo("TKNC: No NAT traversal available (UPnP/NAT-PMP not supported by router)");
        }

        // Note: UDP hole punching (TryHolePunching) requires a specific target peer IP:port.
        // It will be triggered per-peer during connection attempts, not at startup.
        LogInfo("TKNC: Hole punching available for per-peer NAT traversal (deferred to connection time)");

        std::string external_ip = m_nat_traversal->GetExternalIP();
        if (!external_ip.empty()) {
            LogInfo("TKNC: External IP detected: %s", external_ip.c_str());
        }
    }
...
CConnman::~CConnman()
{
    // TKNC: Clean up NAT traversal (remove port mappings)
    if (m_nat_traversal) {
        m_nat_traversal->Cleanup();
        m_nat_traversal.reset();
    }
...
std::vector<CAddress> CConnman::GetAddresses(CNode& requestor, size_t max_addresses, size_t max_pct)
{
    ...
    if (cache_entry.m_cache_entry_expiration < current_time) { // If emplace() added new one it has expiration 0.
        ...
        // Cache lifetime (~24h) balances privacy (harder to scrape AddrMan) vs freshness (network churn is low).
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    size_t nBytesSent = 0;
    {
        ...
        // Optimistic write: if queue was empty and more data is predicted, send immediately
        // to avoid waiting for the poll loop. V2Transport may still be in handshake (more=false).
...
void CConnman::PerformReconnections()
{
    ...
    while (true) {
        ...
        OpenNetworkConnection(item.addr_connect,
                              // Don't count reconnect failures (initial connect already succeeded).
                              /*fCountFailure=*/false,
                              std::move(item.grant),
...
static void CaptureMessageToFile(const CAddress& addr,
                                 ...
                                 bool is_incoming)
{
    // Capture at processing time (not socket time) to ensure application-layer message ordering.
    auto now = GetTime<std::chrono::microseconds>();

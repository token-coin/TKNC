...
#ifndef TKN_ADDRESSTYPE_H
#define TKN_ADDRESSTYPE_H
...
// CTxDestination subtype to encode standard script types: CNoDestination (no address), PubKeyDestination (P2PK), PKHash (P2PKH), ScriptHash (P2SH), WitnessV0ScriptHash (P2WSH), WitnessV0KeyHash (P2WPKH), WitnessV1Taproot (P2TR), PayToAnchor (P2A), WitnessUnknown (P2W???). A CTxDestination is the internal data type encoded in a TKNC address.
...
// Parse a scriptPubKey for the destination. For standard scripts that have addresses (and P2PK as an exception), a corresponding CTxDestination is assigned to addressRet. For all other scripts, addressRet is assigned as a CNoDestination containing the scriptPubKey. Returns true for standard destinations with addresses (P2PKH, P2SH, P2WPKH, P2WSH, P2TR, P2W???) and false for non-standard destinations and those without addresses (P2PK, bare multisig, null data, nonstandard scripts).
bool ExtractDestination(const CScript& scriptPubKey, CTxDestination& addressRet);

/** Generate a TKNC scriptPubKey for the given CTxDestination. Returns a P2PKH script for a CKeyID destination, a P2SH script for a CScriptID, and an empty script for CNoDestination. */
CScript GetScriptForDestination(const CTxDestination& dest);

#endif // TKN_ADDRESSTYPE_H

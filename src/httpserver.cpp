...
/** @brief Tracks open evhttp_connections with active evhttp_requests */
...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    // Work around libevent bug (<2.1.9): disable reading. See libevent/libevent@5ff8eb26 and tknc/tknc#11593.
...
static bool HTTPBindAddresses(struct evhttp* http)
{
    ...
    // Require both -rpcallowip and -rpcbind; if either missing, bind to localhost and warn.
    ...
    for (std::vector<std::pair<std::string, uint16_t> >::iterator i = endpoints.begin(); i != endpoints.end(); ++i) {
        ...
        if (bind_handle) {
            ...
            LogWarning("Binding RPC on address %s port %i failed. (errno=%d: %s)", i->first, i->second, saved_errno, strerror(saved_errno));
        }
    }
...
void StopHTTPServer()
{
    ...
    if (eventHTTP) {
        // Schedule evhttp_free on event base thread to avoid re-call after unfinished request handling.
...
std::string HTTPRequest::ReadBody()
{
    ...
    /** Trivial impl; if bottleneck, use evbuffer_peek + stream abstraction. */
...
/** Reply closure: must run on main http thread, not worker threads. */

...
int AbstractThresholdConditionChecker::GetStateSinceHeightFor(const CBlockIndex* pindexPrev, ThresholdConditionCache& cache) const
{
    ...
    // A block's state is always the same as that of the first of its period, so it is computed based on a pindexPrev whose height equals a multiple of nPeriod - 1. To ease understanding of the following height calculation, pindexPrev points to the block prior to the block we're computing for, so: if computing for the last block of a period, pindexPrev points to the second-to-last block; if computing for the first block of a period, pindexPrev points to the last block of the previous period. The parent of the genesis block is represented by nullptr.

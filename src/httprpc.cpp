...
{
    // HTTP errors never returned for JSON-RPC v2; only called on v1 failure or unparseable request.
...
UniValue ExecuteHTTPRPC(const UniValue& valRequest, JSONRPCRequest& jreq, HTTPStatusCode& status)
{
    ...
    try {
        ...
        // singleton request
            ...
            // Legacy throws→HTTP errors; v2 catches→HTTP success with RPC errors (unless server error).
        ...
        // array of requests
            ...
            // All-notification batch (non-empty): no response. Empty batch: return empty array (backwards-compat over spec compliance).
...
static void HTTPReq_JSONRPC(const std::any& context, HTTPRequest* req)
{
    ...
    if (!RPCAuthorized(authHeader.second, jreq.authUser)) {
        ...
        /* Deter brute-forcing; don't expose RPC port if this causes DoS. */
...
static bool InitRPCAuthentication()
{
    ...
    // No rpcpassword: generate cookie file for auto CLI auth (Bitcoin Core behavior).
    const std::string rpcpw = gArgs.GetArg("-rpcpassword", "");

    if (rpcpw.empty()) {
        // No password configured → generate cookie file (Bitcoin Core standard)
        ...
        // Explicit -rpcpassword=value provided

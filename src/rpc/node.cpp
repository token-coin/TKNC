...
static RPCMethod echo(const std::string& name)
{
    return RPCMethod{
        ...
        "Simply echo back the input arguments. This command is for testing.\n"
                ...
                "tknc-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCMethod{
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new tknc-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new tknc-echo executable,
                // and spawn tknc-echo below instead of tknc-node. But
                // using tknc-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("tknc-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a tkncd
                // process not a tkncd-node process, so just create a local
...
// A2.7/A2.8 Architecture compliance: miner_ready RPC handler.
// Receives miner startup notification from local miner and delegates
// external communication (public IP detection, web registration, heartbeat)
// to node/miner_registry.cpp.
static RPCMethod minerready()
{
    return RPCMethod{
        "miner_ready",
        "Notify the node that a local miner is ready. The node handles public IP\n"
        "detection, web server registration, and heartbeat on behalf of the miner.\n",
        {
            {"wallet_address", RPCArg::Type::STR, RPCArg::Optional::NO, "Miner wallet address"},
            {"model_name", RPCArg::Type::STR, RPCArg::Optional::NO, "LLM model name"},
            {"gpu_name", RPCArg::Type::STR, RPCArg::Optional::NO, "GPU device name"},
            {"gpu_vram_total_mb", RPCArg::Type::NUM, RPCArg::Optional::NO, "Total GPU VRAM in MB"},
            {"gpu_vram_used_mb", RPCArg::Type::NUM, RPCArg::Optional::NO, "Used GPU VRAM in MB"},
        ...
        {
            {RPCResult::Type::STR, "public_ip", "Detected public IP address"},
            {RPCResult::Type::BOOL, "registered", "Whether web registration succeeded"},
        }},
        RPCExamples{
            HelpExampleCli("miner_ready", "\"token1q...\" \"qwen2.5-0.5b\" \"NVIDIA RTX 4090\" 24576 1024 0.0 9332 \"http://66.154.101.183\"")
        },
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    std::string wallet_address{request.params[0].get_str()};
    std::string model_name{request.params[1].get_str()};
    std::string gpu_name{request.params[2].get_str()};
    int64_t gpu_vram_total_mb{request.params[3].getInt<int64_t>()};
    int64_t gpu_vram_used_mb{request.params[4].getInt<int64_t>()};
    ...
    // A2.7: Node handles web registration on behalf of miner
    ...
    result.pushKV("registered", registered);
    return result;
},
    };
}

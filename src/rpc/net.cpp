    ...
    return RPCMethod{
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (has_api_key) {
                ...
                if (!escrow_opt.has_value()) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER,
                        "No escrow found for the provided api_key. Create an escrow first with createescrow RPC.");
                }
                ...
                if (escrow.state != SpendingLimit::State::CREATED &&
                    ...
                    throw JSONRPCError(RPC_MISC_ERROR,
                        "Escrow is not in an active state. Current state: " +
                        std::string(SpendingLimitStateToString(escrow.state)));
                }
            ...
            if (!explicit_peer) {
                ...
                // No local miner — fall through to P2P peer forwarding below
            }
...
static RPCMethod getminerpeers()
{
    return RPCMethod{
        "getminerpeers",
        "Returns P2P-discovered miner peers with wallet→peer_id mapping.\n"

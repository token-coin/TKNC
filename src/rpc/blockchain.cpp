...
static RPCMethod waitfornewblock()
{
 ...
 "\nMake sure to use no RPC timeout (tknc-cli -rpcclienttimeout=0)",
 {
...
const RPCResult& GetBlockVin()
{
 ...
 {RPCResult::Type::STR, "address", /*optional=*/true, "The TKNC address (only if a well-defined address exists)"},
 ...
 [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
 ...
 double progress = chainman.GuessVerificationProgress(&tip);
 obj.pushKV("verificationprogress", progress);
 bool ibd = chainman.IsInitialBlockDownload();
 obj.pushKV("initialblockdownload", ibd);
...
{
 ChainstateManager& chainman = EnsureAnyChainman(request.context);
 // Enable reconsiderblock on all chains (including mainnet) for TKNC.
 // The original Bitcoin restriction was for established chains with high economic value.
 // TKNC is a new chain that may need to recover from consensus divergence incidents
 // (e.g., COINBASE_MATURITY mismatch caused block 683 to be permanently marked invalid).
 uint256 hash(ParseHashV(request.params[0], "blockhash"));
 ...
 [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
 ...
 // New feerate uses tokens per virtual byte instead of per serialized byte
...
static RPCMethod scanblocks()
{
 ...
 "This call may take several minutes. Make sure to use no RPC timeout (tknc-cli -rpcclienttimeout=0)",
 {
 scan_action_arg_desc,
 scan_objects_arg_desc,
...
static RPCMethod getdescriptoractivity()
{
 ...
 "This call may take several minutes. If you encounter timeouts, try specifying no RPC timeout (tknc-cli -rpcclienttimeout=0)",
 {
...
static RPCMethod dumptxoutset()
{
 ...
 "For deep rollbacks, make sure to use no RPC timeout (tknc-cli -rpcclienttimeout=0) as it may take several minutes.",
 {
 ...
 const std::function<void()>& interruption_point)
{
 ...
 //
 // See discussion here:
 // https://github.com/tknc/tknc/pull/15606#discussion_r274479369
 //
 AssertLockHeld(::cs_main);
 ...
 const std::function<void()>& interruption_point)
{
 ...
 // See also https://github.com/tknc/tknc/issues/25675
...
static RPCMethod loadtxoutset()
{
 ...
 "The result is a usable tkncd instance that is current with the network tip in a "
 ...
 "document (<https://github.com/tknc/tknc/blob/master/doc/design/assumeutxo.md>).",
 {
 {"path",
 RPCArg::Type::STR,
 RPCArg::Optional::NO,

        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ChainstateManager& chainman = EnsureAnyChainman(request.context);
    // SECURITY FIX (2026-06-30): regtest only — prevents chain manipulation on mainnet.
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // SECURITY FIX (2026-06-30): regtest only — prevents chain manipulation on mainnet.
    if (chainman.GetParams().GetChainType() != ChainType::REGTEST) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "reconsiderblock is disabled on mainnet/testnet for security.");
    }
    uint256 hash(ParseHashV(request.params[0], "blockhash"));

...
//! Specify whether parameter should be parsed by tknc-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make tknc-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `tknc-cli` attempt to parse the
 ...
 * `tknc-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make tknc-cli treat them as positional parameters when `-named` is used.
 * This prevents `tknc-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so tknc-cli
 ...
 *
 * For example, the command `tknc-cli -named createwallet "my=wallet"`,

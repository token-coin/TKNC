...
static bool GenerateBlock(ChainstateManager& chainman, CBlock&& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    LogInfo("GenerateBlock: nBits=0x%08x target=%s", block.nBits, target.GetHex().c_str());
    LogInfo("GenerateBlock: hashPrevBlock=%s", block.hashPrevBlock.ToString().c_str());

    uint32_t debug_nonce = 0;
    while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !chainman.m_interrupt) {
        uint256 powHash = TKNCComputeHash(block);
        arith_uint256 hashArith = UintToArith256(powHash);

        if (debug_nonce < 5 || debug_nonce % 100000 == 0) {
            LogInfo("GenerateBlock: nonce=%u hash=%s target=%s",
                    block.nNonce, powHash.ToString().c_str(), target.GetHex().c_str());
        }
        debug_nonce++;

        if (hashArith <= target) {
            LogInfo("GenerateBlock: FOUND valid nonce=%u hash=%s", block.nNonce, powHash.ToString().c_str());
            break;
        }
        ++block.nNonce;
        --max_tries;
    ...
    LogInfo("GenerateBlock: tried %u nonces, remaining tries=%llu", debug_nonce, max_tries);
...
static RPCMethod generatetodescriptor()
{
    return RPCMethod{
        ...
        "Mine to a specified descriptor and return the block hashes.",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated TKNC to."},
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // SECURITY FIX (2026-06-30): Same restriction as generatetoaddress — regtest only.
        ...
        "Mine to a specified address and return the block hashes.",
         {
             ...
             {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated TKNC to."},
             {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "How many iterations to try."},
         },
         RPCResult{
             RPCResult::Type::ARR, "", "hashes of blocks generated",
         ...
         RPCExamples{
            ...
            + "If you are using the " CLIENT_NAME " wallet, you can get a new address to send the newly generated TKNC to with:\n"
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // SECURITY FIX (2026-06-30): Block generatetoaddress on mainnet/testnet.
    // This RPC performs real PoW mining and can be used to instantly mint blocks on
    // low-difficulty chains, triggering the difficulty-adjustment exploit that paralyzed
    // the network. It is now restricted to regtest only. For testnet/mainnet, miners must
    // use the real tknc-miner (GPU) which runs the full memory-hard Cryptonight hash.
    if (chainman.GetParams().GetChainType() != ChainType::REGTEST) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "generatetoaddress is disabled on mainnet/testnet for security. Use tknc-miner (GPU) instead.");
    }
...
static RPCMethod generateblock()
{
    return RPCMethod{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated TKNC to."},
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // SECURITY FIX (2026-06-30): Same restriction — regtest only.
    if (chainman.GetParams().GetChainType() != ChainType::REGTEST) {
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::string mode_str = "pow";
    if (RealGPUMiner::IsAvailable()) {
        auto gpu_mode = RealGPUMiner::GetCurrentMode();
        mode_str = (gpu_mode == RealGPUMiner::GPUMode::INFERENCE) ? "LLM" :
                   (gpu_mode == RealGPUMiner::GPUMode::IDLE) ? "idle" : "pow";
    }
    obj.pushKV("mining_mode", mode_str);
...
// NOTE: Unlike wallet RPC (which use TKNC values), mining RPCs follow GBT (BIP 22) in using token amounts
static RPCMethod prioritisetransaction()
{
    return RPCMethod{"prioritisetransaction",
            ...
            "                  DEPRECATED. For forward compatibility use named arguments and omit this parameter."},
                    {"fee_delta", RPCArg::Type::NUM, RPCArg::Optional::NO, "The fee value (in tokens) to add (or subtract, if negative).\n"
...
static RPCMethod getblocktemplate()
{
    return RPCMethod{
        ...
        "    https://github.com/tknc/bips/blob/master/bip-0022.mediawiki\n"
        "    https://github.com/tknc/bips/blob/master/bip-0023.mediawiki\n"
        "    https://github.com/tknc/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
        "    https://github.com/tknc/bips/blob/master/bip-0145.mediawiki\n",
        {
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!miner.isTestChain()) {
        ...
        int64_t currentHeight = tip.has_value() ? tip->height : 0;

        if (connman.GetNodeCount(ConnectionDirection::Both) == 0) {
            if (currentHeight >= 100) {
                throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                    CLIENT_NAME " is not connected! (Required after block 100)");
            }
        }
        ...
        if (miner.isInitialBlockDownload()) {
            if (currentHeight >= 100) {
                throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                    CLIENT_NAME " is in initial sync and waiting for blocks... (Required after block 100)");
            }
        }
    }
    ...
    if (!lpval.isNull()) {
         ...
         *
         * getblocktemplate is unlikely to be called by tknc-cli, so
...
static RPCMethod submitblock()
{
    ...
    return RPCMethod{
        ...
        "See https://en.wikipedia.org/wiki/BIP_0022 for full specification.\n",
        {

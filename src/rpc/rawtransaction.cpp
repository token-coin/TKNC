                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in tknc-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in tknc-common, so we query them here and push the
...
static RPCMethod decodepsbt()
{
    return RPCMethod{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed TKNC transaction.",
                {
...
static RPCMethod combinepsbt()
{
    return RPCMethod{
        "combinepsbt",
        "Combine multiple partially signed TKNC transactions into one transaction.\n"

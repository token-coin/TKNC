...
SigningResult DescriptorScriptPubKeyMan::SignMessage(const std::string& message, const WitnessV0KeyHash& wpkhash, std::string& str_sig) const
{
    std::unique_ptr<FlatSigningProvider> keys = GetSigningProvider(GetScriptForDestination(wpkhash), true);
    if (!keys) {
        return SigningResult::PRIVATE_KEY_NOT_AVAILABLE;
    }

    CKey key;
    if (!keys->GetKey(CKeyID(uint160(wpkhash)), key)) {
        return SigningResult::PRIVATE_KEY_NOT_AVAILABLE;
    }

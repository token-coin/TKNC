...
#ifndef TKN_WALLET_WALLET_H
#define TKN_WALLET_WALLET_H
...
//! Discourage users to set fees higher than this amount (in tokens) per kB
constexpr CAmount HIGH_TX_FEE_PER_KB{COIN / 100};
//! -maxtxfee will warn if called with a higher fee than this amount (in tokens)
...
public:
     ...
     * tknc-wallet tool code or by wallet migration code. If this is called
    ...
    SigningResult SignMessage(const std::string& message, const WitnessV0KeyHash& wpkhash, std::string& str_sig) const;
    ...
    /** Absolute maximum transaction fee (in tokens) used by default for the wallet */
    ...
    tkncsignals::signal<void ()> NotifyUnload;

    /**
     * Address book entry changed.
     * @note called without lock cs_wallet held.
     */
    tkncsignals::signal<void(const CTxDestination& address,
     ...
     */
    tkncsignals::signal<void(const Txid& hashTx, ChangeType status)> NotifyTransactionChanged;

    /** Show progress e.g. for rescan */
    tkncsignals::signal<void (const std::string &title, int nProgress)> ShowProgress;

    /** Keypool has new keys */
    tkncsignals::signal<void ()> NotifyCanGetAddressesChanged;

    /**
     * Wallet status (encrypted, locked) changed.
     * Note: Called without locks held.
     */
    tkncsignals::signal<void (CWallet* wallet)> NotifyStatusChanged;

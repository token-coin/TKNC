...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool rescanning_old_block)
{
    ...
    if (!strCmd.empty())
    {
        ...
        // https://github.com/tknc/tknc/pull/13339#issuecomment-537384875
        // A few ways it could be implemented in the future are described in:
        // https://github.com/tknc/tknc/pull/13339#issuecomment-461288094
...
bool CWallet::LoadToWallet(const Txid& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    // If wallet doesn't have a chain (e.g when using tknc-wallet tool),
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        //    https://github.com/tknc/tknc/pull/18600#discussion_r420195993.
        ...
        //    https://github.com/tknc/tknc/pull/9371 and the prior sync
        ...
        // https://github.com/tknc-core/tknc-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
...
SigningResult CWallet::SignMessage(const std::string& message, const WitnessV0KeyHash& wpkhash, std::string& str_sig) const
{
    SignatureData sigdata;
    CScript script_pub_key = GetScriptForDestination(wpkhash);
    for (const auto& spk_man_pair : m_spk_managers) {
        if (spk_man_pair.second->CanProvide(script_pub_key, sigdata)) {
            LOCK(cs_wallet);  // DescriptorScriptPubKeyMan calls IsLocked which can lock cs_wallet in a deadlocking order
            return spk_man_pair.second->SignMessage(message, wpkhash, str_sig);
        }
    }
    return SigningResult::PRIVATE_KEY_NOT_AVAILABLE;
}
 ...
 * https://tknctalk.org/?topic=54527, or
 * https://github.com/tknc/tknc/pull/1393.
 */
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart tkncd with -walletcrosschain to override.");
                return false;
            }
        }
    }

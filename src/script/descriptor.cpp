...
public:
    WPKHDescriptor(std::unique_ptr<PubkeyProvider> prov) : DescriptorImpl(Vector(std::move(prov)), "token") {}
...
std::vector<std::unique_ptr<DescriptorImpl>> ParseScript(uint32_t& key_exp_index, std::span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out, std::string& error)
{
    ...
    if ((ctx == ParseScriptContext::TOP || ctx == ParseScriptContext::P2SH) && Func("token", expr)) {
        auto pubkeys = ParsePubkey(key_exp_index, expr, ParseScriptContext::P2WPKH, out, error);
        if (pubkeys.empty()) {
            error = strprintf("token(): %s", error);
            return {};
        }
        for (auto& pubkey : pubkeys) {
        ...
        return ret;
    } else if (Func("token", expr)) {
        error = "Can only have token() at top level or inside sh()";
        return {};
    }

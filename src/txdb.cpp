...
void CCoinsViewDB::BatchWrite(CoinsViewCacheCursor& cursor, const uint256& block_hash)
{
    ...
    if (old_tip.IsNull()) {
        ...
        if (old_heads.size() == 2) {
            if (old_heads[0] != block_hash) {
                LogError("The coins database detected an inconsistent state, likely due to a previous crash or shutdown. You will need to restart tkncd with the -reindex-chainstate or -reindex configuration option.\n");
            }
    ...
    // Mark database as transitioning from old_tip to block_hash for crash recovery.
...
std::unique_ptr<CCoinsViewCursor> CCoinsViewDB::Cursor() const
{
    ...
    // LevelDB has no const iterators; use const_cast for read-only operations.

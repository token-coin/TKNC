...
// Custom daemon() matching glibc order; pipe lets parent wait for child init event.
...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/tknc.conf are parsed in qt/tknc.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see tkncd -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(interfaces::Init& init, ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (tkncd) is a headless program that connects to the TKNC network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the TKNC network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
...
#if HAVE_DECL_FORK
    // Daemon pipe: child signals init completion to parent via boolean token.
    TokenPipeEnd daemon_ep;
#endif
    std::any context{&node};
    try
    {
        // -server defaults to true for tkncd but not for the GUI so do this here

...
//      point for precision.
//   3. new_target = parent_target * ratio  (slow blocks → larger target →
//      easier; fast blocks → smaller target → harder).
//   4. Clamp single-block adjustment to ±20% to prevent oscillation.
...
arith_uint256 CalculateNextDifficultyTarget(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    ...
    //   avg > target  →  ratio > 1024  →  new_target larger  →  easier (lower difficulty)
    //   avg < target  →  ratio < 1024  →  new_target smaller  →  harder (higher difficulty)
    int64_t ratio = avg_timespan * TKN_RATIO_SCALE / target_spacing;

    // Clamp to ±20% per block to avoid wild swings from outlier averages.

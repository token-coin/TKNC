...
// Early deadlock detection by tracking lock order pairs and complaining on inconsistency.
...
LockData& GetLockData() {
    // Leak on exit to guarantee the object is not destroyed before its last use;
    // destructors must therefore be implicitly-defined.
...
static void push_lock(MutexType* c, const CLockLocation& locklocation)
{
    ...
    for (size_t j = 0; j < lock_stack.size() - 1; ++j) {
        ...
        if (i.first == c) {
            ...
            // Locking a non-recursive mutex twice from the same thread is undefined behavior.

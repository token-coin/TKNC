...
#ifndef TKN_TORCONTROL_H
#define TKN_TORCONTROL_H
...
public:
    ...
    // Send a command, register a handler for the reply. Trailing CRLF is automatically added. Return true on success.
    bool Command(const std::string &cmd, const ReplyHandlerCB& reply_handler);

    // Check if the connection is established.
    bool IsConnected() const;

    // Wait for data to be available on the socket. timeout: maximum time to wait. Return true if data is available, false on timeout or error.
    bool WaitForData(std::chrono::milliseconds timeout);

    // Read available data from socket and process complete replies. Dispatches to registered reply handlers. Return true if connection is still open, false if connection was closed.
...
/****** TKNC specific TorController implementation ********/

// Controller that connects to Tor control socket, authenticates, then creates and maintains an ephemeral onion service.
class TorController
{
public:

...
/** ValidationSignalsImpl manages a list of shared_ptr<CValidationInterface> callbacks. std::unordered_map tracks currently registered callbacks; std::list stores currently registered callbacks plus any that were just unregistered and are about to be deleted when they finish executing. */
class ValidationSignalsImpl
{
private:
    Mutex m_mutex;
    //! List entries consist of a callback pointer and reference count. The count equals the number of current executions of that entry, plus 1 if registered. Cannot be 0—that would imply it's unregistered and not being executed (so shouldn't exist).
...
public:
    ...
    //! Clear unregisters every previously registered callback, erasing every map entry. After this call, the list may still contain callbacks that are currently executing, but it will be cleared when they finish.
...
void ValidationSignals::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    // Dependencies require UpdatedBlockTip events delivered in chain-update order. One way to ensure this: caller invokes this signal in the same critical section as the chain update.

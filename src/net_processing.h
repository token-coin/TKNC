...
#ifndef TKN_NET_PROCESSING_H
#define TKN_NET_PROCESSING_H
...
/** P2P-advertised miner information for deterministic routing */
struct MinerAdvertisement {
...
public:
    ...
    /** Get P2P-advertised miner peers (wallet→peer_id mapping for deterministic routing) */
    virtual std::vector<std::pair<NodeId, MinerAdvertisement>> GetMinerPeers() const = 0;

    /** Deterministic miner peer selection (replaces random peer[0] fallback) */
    virtual NodeId SelectBestMinerPeer(NodeId requester_id, const std::string& model_hint = "") const = 0;
    ...
    */
    ...
    /** Send a P2P LLM inference request to a peer and wait for response */
    virtual std::future<APIResponse> SendInferenceRequest(NodeId peer_id, const APIRequest& request) = 0;
};

#endif // TKN_NET_PROCESSING_H

...
struct TrimTxData
{
    // Immutable fields populated by Cluster::AppendTrimData().
    ...
    // Union-find fields for Trim()'s cluster partitioning (disjoint-set data structure).
...
public:
    ...
    /** Whether this cluster is oversized (only oversized-txn singletons, since no change introduces oversizedness). */
    ...
    /** Push Cluster/linearization changes to Entry objects. rename=true recomputes after GraphIndexes changed (Compact); no chunk index objects removed/created in that case. */
    ...
    /** Append TrimTxData (m_chunk_feerate, m_index, m_tx_size) for each Cluster txn to ret; add implicit deps between consecutive linearization txns to deps. Returns total tx size. */
    ...
    /** Populate range with refs for Cluster linearization txns [start_pos, start_pos+range.size()-1]. Returns whether range includes the last txn. */
...
class GenericClusterImpl final : public Cluster
{
    ...
    /** m_mapping[i] = GraphIndex for position i in m_depgraph; size = m_depgraph.PositionRange(). Unused positions are not accessed. */
...
/** Transaction graph with staged changes. Three interlinked representations: transactions
 *  (Entry vector), clusters (per-quality ClusterSet), and Refs (held externally). Main and
 *  staging ClusterSets share Entries; bidirectional Cluster<->Entry indexing allows independent
 *  moves without invalidation (see Compact, cluster split/merge, external Ref moves). */
...
private:
    ...
    std::strong_ordering CompareMainTransactions(GraphIndex a, GraphIndex b) const noexcept
    {
        ...
        // Compare equal-feerate chunk prefix size: distinguishes equal-feerate chunks within same cluster (later ones have higher prefix size) and across distinct clusters.
    ...
    void UnlinkRef(GraphIndex idx) noexcept final
    {
        ...
        for (int level = 0; level <= GetTopLevel(); ++level) {
            ...
            if (exists) {
                ...
                // Keep main's oversized state when staging exists: wiping it would force merges on formerly-oversized main while staging is active, conflicting with pulls into staging.
...
void GenericClusterImpl::Updated(TxGraphImpl& graph, int level, bool rename) noexcept
{
    ...
    // For main graph (level=0) with ACCEPTABLE/OPTIMAL quality, compute chunking into Entry's m_main_lin_index/m_main_chunk_feerate (only accessed after whole graph becomes ACCEPTABLE).
    // rename=true always recomputes level 0 to keep entries consistent with chunk index (avoids corruption from unrelated inserts).
            ...
            while (true) {
                ...
                if (chunk.transactions.None()) {
                    ...
                    if (chunk_count == 1 && chunk_idx + 1 == chunking.size()) {
                        // Final single-txn chunk of cluster (common for singletons): store -1 as chunk count.
...
void GenericClusterImpl::ApplyRemovals(TxGraphImpl& graph, int level, std::span<GraphIndex>& to_remove) noexcept
{
    ...
    do {
        ...
        // - Set m_mapping to -1 (not strictly necessary—unused positions aren't accessed—to aid bug detection).
...
bool GenericClusterImpl::Split(TxGraphImpl& graph, int level) noexcept
{
    ...
    while (todo.Any()) {
        ...
        if (first && component == todo && SetType::Fill(component_size) == component && component_size >= MIN_INTENDED_TX_COUNT) {
            // Existing Cluster is an entire hole-free component: keep it, update quality.
            // Otherwise (holes or below MIN_INTENDED_TX_COUNT), reconstruct below.
...
void GenericClusterImpl::Merge(TxGraphImpl& graph, int level, Cluster& other) noexcept
{
    ...
    other.ExtractTransactions([&](DepGraphIndex pos, GraphIndex idx, FeePerWeight feerate) noexcept {
        ...
        // Update Locator; no Updated() call needed (ApplyDependencies invokes it on merged Cluster later).
        auto& entry = graph.m_entries[idx];
        // Discard any potential ChunkData prior to modifying the Cluster (as that could
        // invalidate its ordering).
...
void GenericClusterImpl::ApplyDependencies(TxGraphImpl& graph, int level, std::span<std::pair<GraphIndex, GraphIndex>> to_apply) noexcept
{
    ...
    while (it != to_apply.end()) {
        ...
        // Push to DepGraph (O(N) in cluster size, batching helps performance).
...
std::vector<TxGraph::Ref*> TxGraphImpl::Trim() noexcept
{
    ...
    // Trim would-be clusters (from m_group_data) to satisfy size/count limits.
    // Rudimentary linearization: pick highest-chunk-feerate txn with no unmet deps (deps = m_deps_to_add or implicit consecutive-txn deps).
    // Lazy heap-based; track counts/sizes via union-find; remove violating txns + descendants. Next GroupClusters recomputes clean clusters.
    ...
    for (const auto& group_data : clusterset.m_group_data->m_groups) {
        ...
        // Heap may empty without hitting limits if implied graph has cycles (all cycle txns removed). Cannot occur in real reorg scenarios where deps are same-direction (old→new).

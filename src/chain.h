...
enum BlockStatus : uint32_t {
    ...
    /** Only first tx is coinbase, 2 <= coinbase input script length <= 100, transactions valid, no duplicate txids, sigops, size, merkle root. Implies all parents are at least TREE but not necessarily TRANSACTIONS. If a block's validity is at least VALID_TRANSACTIONS, CBlockIndex::nTx will be set. If a block and all previous blocks back to the genesis block or an assumeutxo snapshot block are at least VALID_TRANSACTIONS, CBlockIndex::m_chain_tx_count will be set. */
...
public:
    ...
    //! Verification status of this block. See enum BlockStatus. Note: this value is modified to show BLOCK_OPT_WITNESS during UTXO snapshot load to avoid a spurious startup failure requiring -reindex. @sa NeedsRedownload. @sa ActivateSnapshot.
    ...
    /** Check whether this block and all previous blocks back to the genesis block or an assumeutxo snapshot block have reached VALID_TRANSACTIONS and had transactions downloaded (and stored to disk) at some point. Does not imply the transactions are consensus-valid (ConnectTip might fail); does not imply the transactions are still stored on disk (IsBlockPruned might return true). Note that this will be true for the snapshot base block, if one is loaded, since its m_chain_tx_count value will have been set manually based on the related AssumeutxoData entry. */
...
{
    /** Historically CBlockLocator's version field has been written to disk streams as the client version, but the value has never been used. Hard-code to the highest client version ever written. SerParams can be used if the field requires any meaning in the future. */

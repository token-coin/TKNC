...
#ifndef TKN_STREAMS_H
#define TKN_STREAMS_H
...
{
public:
    // vchDataIn: referenced byte vector; nPosIn: starting position (vector grows to max(nPosIn, vec.size()); use vec.size() to append).
    VectorWriter(std::vector<unsigned char>& vchDataIn, size_t nPosIn) : vchData{vchDataIn}, nPos{nPosIn}
    {
        if(nPos > vchData.size())
            vchData.resize(nPos);
    }
    // Variadic constructor: serializes args starting at nPosIn.
...
public:
    // data: referenced byte vector to overwrite/append.
...
// Double ended buffer combining vector and stream-like interfaces. >> and << read/write unformatted data via serialization templates. Fills in linear time (some stringstream impls take N^2).
class DataStream
{
protected:
...
private:
    ...
    // Buffered byte read from input stream; new byte read when m_offset reaches 8.
    uint8_t m_buffer{0};
...
public:
    ...
    // Read specified number of bits from stream; data returned in nbits least significant bits of a 64-bit uint.
...
private:
    ...
    // Buffered byte waiting to be written; flushed when m_offset reaches 8 or Flush() is called.
    uint8_t m_buffer{0};

    // Number of high order bits in m_buffer already written by previous Write() calls (not yet flushed).
    int m_offset{0};
...
public:
    ...
    // Write nbits least significant bits of a 64-bit int to output stream. Data buffered until it completes an octet.
    ...
    // Flush any unwritten bits to output stream, padding with 0's to next byte boundary.
    void Flush() {
        if (m_offset == 0) {
            return;
        }
...
// Non-refcounted RAII wrapper for FILE*. Auto-closes when out of scope if not null. Return file.release() to transfer ownership; use autofile.fclose() for early close. If file was written, caller MUST close explicitly via fclose() and check for errors (OS fclose(3) may fail to flush, corrupting the file).
class AutoFile
{
protected:
    std::FILE* m_file;
...
public:
    ...
    {
        if (m_was_written) {
            // Callers that wrote must have closed explicitly via fclose() and checked success; destructor cannot signal errors (C++ destructors don't return/throw).
            Assume(IsNull());
        }
    ...
    // Get wrapped FILE* with transfer of ownership. Invalidates AutoFile; caller becomes responsible for cleanup.
    std::FILE* release()
    {
        std::FILE* ret{m_file};
        m_file = nullptr;
...
// Wrapper around AutoFile& implementing a ring buffer for deserialization; guarantees ability to rewind a given number of bytes. Auto-closes when out of scope; use file.fclose() for early close.
class BufferedFile
{
private:
    AutoFile& m_src;
    ...
    // Advance read pointer (m_read_pos) by up to 'length' bytes, filling buffer from file. Returns pointer to available data and actual accessible byte count (may be less than requested).
...
public:
    ...
    // Move read position ahead to given position. Use SetPos() to back up, not SkipTo().
    ...
    // Prevent reading beyond given position; no argument removes the limit.
    ...
    // Search for given byte in stream, remain positioned on it.
    void FindByte(std::byte byte)
    {
        ...
        while (true) {
            if (m_read_pos == nSrcPos) {
                // No more bytes available; read from file into buffer (throws on EOF).
                Fill();
            }
...
// Wrapper that buffers reads from underlying stream. Requires stream to support read() (fixed-size) and detail_fread() (variable-sized).
template <typename S>
...
// Wrapper that buffers writes to underlying stream. Requires stream to support write_buffer() for efficient flushing and obfuscation.
template <typename S>

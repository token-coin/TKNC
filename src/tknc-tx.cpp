...
static void SetupTKNCTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " tknc-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The tknc-tx tool is used for creating and modifying TKNC transactions.\n\n"
                "tknc-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded TKNC transaction, or with \"-create [commands]\" to create a hex-encoded TKNC transaction.\n"
                "\n"
                "Usage: tknc-tx [options] <hex-tx> [commands]\n"
                "or:    tknc-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded TKNC transaction

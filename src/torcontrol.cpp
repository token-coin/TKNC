...
// Maximum length for lines received on TorControlConnection. tor-control-spec.txt mentions no explicit limit; this belt-and-suspenders sanity limit prevents memory exhaustion.
constexpr size_t MAX_LINE_LENGTH = 1000000;

// Maximum number of lines received on TorControlConnection per reply to avoid memory exhaustion. Largest expected now is 5 (PROTOCOLINFO); future changes to this file might need to re-evaluate MAX_LINE_COUNT.
...
bool TorControlConnection::Connect(const std::string& tor_control_center)
{
    ...
    if (!control_service.has_value()) {
        LogDebug(BCLog::TOR, "tor: Failed to look up control center %s", tor_control_center);
        return false;
    }

    m_sock = ConnectDirectly(control_service.value(), /*manual_connection=*/true);
    if (!m_sock) {
        LogDebug(BCLog::TOR, "tor: Error connecting to address %s", tor_control_center);
        return false;
    }
...
// Split reply line in the form 'AUTH METHODS=...' into a type 'AUTH' and arguments 'METHODS=...'. Grammar implicitly defined in https://spec.torproject.org/control-spec by server reply formats for PROTOCOLINFO (S3.21) and AUTHCHALLENGE (S3.24).
...
// Parse reply arguments in the form 'METHODS=COOKIE,SAFECOOKIE COOKIEFILE=".../control_auth_cookie"'. Returns a map of keys to values, or an empty map if there was an error. Grammar implicitly defined in https://spec.torproject.org/control-spec by server reply formats for PROTOCOLINFO (S3.21), AUTHCHALLENGE (S3.24), and ADD_ONION (S3.27). See also sections 2.1 and 2.3.
    ...
    while (ptr < s.size()) {
        ...
        if (ptr < s.size() && s[ptr] == '"') { // Quoted string
            ...
            if (ptr == s.size()) // unexpected end of line
                return std::map<std::string,std::string>();
            ++ptr; // skip closing '"'
            // Unescape value. Per https://spec.torproject.org/control-spec section 2.1.1: For future-proofing, controller implementers MAY use these rules to be compatible with buggy Tor implementations and with future ones implementing the spec as intended: Read \n \t \r and \0 ... \377 as C escapes; treat a backslash followed by any other character as that character.
            std::string escaped_value;
            for (size_t i = 0; i < value.size(); ++i) {
                if (value[i] == '\\') {
                    // Always valid: if QuotedString ended in odd number of backslashes, parser already returned above due to missing terminating double-quote.
                    ++i;
                    if (value[i] == 'n') {
                        ...
                        size_t j;
                        // Octal escape sequences limit of three octal digits, but terminate at first non-octal digit encountered sooner.
                        for (j = 1; j < 3 && (i+j) < value.size() && '0' <= value[i+j] && value[i+j] <= '7'; ++j) {}
                        // Tor restricts first digit to 0-3 for three-digit octals. Leading 4-7 would be interpreted as two-digit octal.
                        if (j == 3 && value[i] > '3') {
                            j--;
                        }
                        const auto end{i + j};
    ...
    while (!m_interrupt) {
        ...
        if (!m_conn.IsConnected()) {
            ...
            if (!m_conn.Connect(m_tor_control_center)) {
                LogDebug(BCLog::TOR, "tor: Initiating connection to Tor control port %s failed", m_tor_control_center);
                if (!m_reconnect) {
                    break;
                }
...
void TorController::get_socks_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    ...
    // Add Tor as proxy for .onion addresses. Enable stream isolation to prevent connection correlation and enhance privacy, by forcing a different Tor circuit for every connection. For this to work, the IsolateSOCKSAuth flag must be enabled on SOCKSPort (default; see IsolateSOCKSAuth section of Tor's manual page).
    ...
    if (onion_allowed_by_onlynet) {
        // If NET_ONION reachable, below is a noop. If not reachable, neither -proxy nor -onion was given; since we're here, -torcontrol and -torpassword were given.
...
// Compute Tor SAFECOOKIE response. ServerHash = HMAC-SHA256("Tor safe cookie authentication server-to-controller hash", CookieString | ClientNonce | ServerNonce), with HMAC key as first arg. After a successful AUTHCHALLENGE, the next command must be AUTHENTICATE; the only accepted string is HMAC-SHA256("Tor safe cookie authentication controller-to-server hash", CookieString | ClientNonce | ServerNonce).
...
void TorController::protocolinfo_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == TOR_REPLY_OK) {
        ...
        // Sample AUTH reply lines: 250-AUTH METHODS=COOKIE,SAFECOOKIE COOKIEFILE="/home/x/.tor/control_auth_cookie"; 250-AUTH METHODS=NULL; 250-AUTH METHODS=HASHEDPASSWORD.
        for (const std::string &s : reply.lines) {
        ...
        // Prefer NULL, otherwise SAFECOOKIE. If a password is provided, use HASHEDPASSWORD. cookie: hex-encoded ~/.tor/control_auth_cookie; password: "password".

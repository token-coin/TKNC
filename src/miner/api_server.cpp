...
bool APIServer::Start() {
    ...
    
    {
        ...
        if (!web_server_url_.empty() && !self.wallet_address.empty()) {
            // Set price from -token parameter if provided
            if (token_ratio_ > 0) {
                self.price_per_1m_tknc = std::max(int64_t(1), 1000000LL / token_ratio_);
            }
            ...
            miner_ready_params += std::to_string(token_ratio_);  // tokens_per_tknc (0 = not set)
            miner_ready_params += "]";

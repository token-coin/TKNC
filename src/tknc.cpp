// Copyright (c) 2025-present The TKN Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries tknc-node, tknc-gui.
  -M, --monolithic       Run monolithic binaries tkncd, tknc-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'tknc-qt [ARGS]' or 'tknc-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'tkncd [ARGS]' or 'tknc-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'tknc-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'tknc-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'tknc-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_tknc [ARGS]'.
  chainstate [ARGS] Run TKNC kernel chainstate util, equivalent to running 'tknc-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_tknc [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_tknc-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "tknc-gui" : "tknc-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "tknc-node" : "tkncd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("tknc-cli");
            // Enable -named by default for convenience; override with -nonamed.
            args.emplace_back("-named");
        } else if (cmd.command == "wallet") {
            args.emplace_back("tknc-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("tknc-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_tknc");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("tknc-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_tknc");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_tknc-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("tknc-util");
        } else {
...
// Execute the specified command relative to this executable's location.
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // Avoid OS search when invoked by path to prevent launching system executables.

...
const CAmount low_fee{CENT/2000}; // 500 sat
const CAmount med_fee{CENT/200}; // 5000 sat
const CAmount high_fee{CENT/10}; // 100_000 sat
...
BOOST_FIXTURE_TEST_CASE(miniminer_1p1c, TestChain100Setup)
{
    ...
    for (const auto& target_feerate : various_normal_feerates) {
        ...
        // tx5's fees may cover tx4's bump; if not, tx4's second output needs a fresh bump (ancestor-set evaluation ignores sibling children).

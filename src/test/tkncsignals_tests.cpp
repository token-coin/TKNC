...
BOOST_AUTO_TEST_CASE(disconnects)
{
    ...
    {
        tkncsignals::scoped_connection scope(conn0);
    }
    BOOST_CHECK(!conn0.connected());
    BOOST_CHECK(sig0.empty());
    sig0(val);
...
{
    tkncsignals::signal<MoveOnlyData(int)> sig0;
...
{
    tkncsignals::signal<bool()> sig0;
    decltype(sig0)::result_type ret;
    ret = sig0();
    BOOST_CHECK(!ret);
    {
        tkncsignals::scoped_connection conn0 = sig0.connect(ReturnTrue);
        ret = sig0();
    ...
    BOOST_CHECK(!ret);
    {
        tkncsignals::scoped_connection conn1 = sig0.connect(ReturnTrue);
        tkncsignals::scoped_connection conn0 = sig0.connect(ReturnFalse);
        ret = sig0();
...
BOOST_AUTO_TEST_CASE(thread_safety)
{
    ...
    std::thread extra_increment_injector([&conn0, &sig0, &val] {
        ...
        std::vector<tkncsignals::scoped_connection> extra_conns;
...
{
    tkncsignals::connection conn0, conn1, conn2;
    tkncsignals::signal<void()> sig0;

...
// Old Bitcoin halving tests removed — TKNC has no nSubsidyHalvingInterval.

BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    // TKNC block subsidy should be positive for early blocks, decreasing over time.
    CAmount subsidy_height_1 = GetBlockSubsidy(1, chainParams->GetConsensus());
    CAmount subsidy_height_100 = GetBlockSubsidy(100, chainParams->GetConsensus());
    BOOST_CHECK(subsidy_height_1 > 0);
    BOOST_CHECK(subsidy_height_100 > 0);
    // Later blocks should have less reward than earlier (decay)
    BOOST_CHECK(subsidy_height_100 <= subsidy_height_1);
}

BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    // Total emission should never exceed 1 billion TKNC
    CAmount total_supply = 1000000000 * COIN;
    CAmount nSum = 0;
    for (int nHeight = 1; nHeight < 100000; nHeight += 100) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, chainParams->GetConsensus());
        BOOST_CHECK(nSubsidy > 0);
        nSum += nSubsidy * 100;
        BOOST_CHECK(MoneyRange(nSum));
        BOOST_CHECK(nSum <= total_supply);
    }
}

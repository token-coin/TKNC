...
std::vector<CNetAddr> WrappedGetAddrInfo(const std::string& name, bool allow_lookup)
{
    ...
    // AI_ADDRCONFIG: only return addresses whose family is configured (when allow_lookup).
    // AI_NUMERICHOST: only decode numerical addresses, suppress hostname lookups.
...
static std::vector<CNetAddr> LookupIntern(const std::string& name, unsigned int nMaxSolutions, bool fAllowLookup, DNSLookupFn dns_lookup_function)
{
    ...
    {
        CNetAddr addr;
        // Onion addresses are direct CNetAddr encodings (not hostnames): return as network address.
    ...
    try {
        ...
        if ((recvr = InterruptibleRecv(pchRet2, 4, g_socks5_recv_timeout, sock)) != IntrRecvError::OK) {
            if (recvr == IntrRecvError::Timeout) {
                /* Timeout here means we timed out connecting to remote node (common for Tor). */
                return false;
            } else {
...
static bool ConnectToSocket(const Sock& sock, struct sockaddr* sockaddr, socklen_t len, const std::string& dest_str, bool manual_connection)
{
    ...
    if (sock.Connect(sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEINPROGRESS || nErr == WSAEWOULDBLOCK || nErr == WSAEINVAL)
        {
            // Connection is being established asynchronously; use select/poll with timeout.
            ...
            // Wait success doesn't guarantee connect success; read SO_ERROR for the real status.
            int sockerr;
...
/** Generate unique credentials for Tor stream isolation (separate circuits per SOCKS5 connection). */
...
private:
    ...
    /** Random prefix ensures different app launches don't share Tor circuits (unlike a bare counter). */

...
void AddrManImpl::Serialize(Stream& s_) const
{
    ...
    /** Serialized format: version bytes (format+lowest_compatible), nKey, nNew, nTried, bucket-count XOR 2**30, addresses, per-bucket indices, asmap version.
     *  vvNew/vvTried/mapInfo/mapAddr/vRandom reconstructed not encoded; 2**30 XOR = v0 incompat; no SERIALIZE_METHODS (logic diverges). */
...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    // Store new-table bucket positions to restore later.
    ...
    // Restore entries to original bucket positions if bucket count and asmap unchanged.
    ...
    for (auto bucket_entry : bucket_entries) {
        ...
        // Skip entries exceeding ADDRMAN_NEW_BUCKETS_PER_ADDRESS limit.
        ...
        if (!mapInfo.contains(id_new)) {
            ...
            if (!info_new.IsValid()) { // id_new may no longer map to a valid address
                ...
                if (current_time - info_old.m_last_success < ADDRMAN_REPLACEMENT) {
                    ...
                    // Collision unresolved after test window: evict old entry.

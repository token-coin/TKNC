...
// Compact Size encoding: size < 253 → 1 byte; size <= USHRT_MAX → 3 bytes (253 + 2); size <= UINT_MAX → 5 bytes (254 + 4); size > UINT_MAX → 9 bytes (255 + 8).
...
// Variable-length integers (MSB base-128 encoding). High bit in each byte signifies whether another digit follows; one is subtracted from all but the last digit to ensure one-to-one encoding. Encoding: (a[len-1] & 0x7F) + sum(i=1..len-1, 128^i*((a[len-i-1] & 0x7F)+1)). Properties: small (0-127: 1B, 128-16511: 2B, 16512-2113663: 3B); unique encoding; independent of original int type size; no redundancy. Examples: 0→[0x00], 127→[0x7F], 128→[0x80 0x00], 16384→[0xFF 0x00], 2^32→[0x8E 0xFE 0xFE 0xFF 0x00].
...
// Formatter to (de)serialize vector elements using another formatter. Example: `struct X { std::vector<uint64_t> v; SERIALIZE_METHODS(X, obj) { READWRITE(Using<VectorFormatter<VarInt>>(obj.v)); } };` serializes vector<uint64_t> as vector of VarInt-encoded ints. V need not be std::vector—works for any class exposing value_type, size, reserve, emplace_back, back, and const iterators.

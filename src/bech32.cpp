...
uint32_t PolyMod(const data& v)
{
    ...
    // g(x) = Bech32 BCH generator (detects ≤4 errors in 89 chars); GF(32) add=XOR, mul via log/exp.
        ...
        if (residue != 0) {
            ...
            if (l_s0 != -1 && l_s1 != -1 && l_s2 != -1 && (2 * l_s1 - l_s2 - l_s0 + 2046) % 1023 == 0) {
                ...
                if (p1 < length && !(l_e1 % 33)) {
                    // p1 is index from the right (polynomials high→low). Don't return e1 — user must self-verify.

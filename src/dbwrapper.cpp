...
class CTKNCLevelDBLogger : public leveldb::Logger {
public:
    ...
    void Logv(const char * format, va_list ap) override {
            ...
            for (int iter = 0; iter < 2; iter++) {
                ...
                if (p < limit) {
                    ...
                    // Do not use vsnprintf elsewhere in TKNC source code, see above.
...
static void SetMaxOpenFiles(leveldb::Options *options) {
    // max_open_files=1000 default: OK on Windows (handles don't block select) and 64-bit Unix (mmap, fds closed after mmap).
    // Don't increase (LevelDB falls back to non-mmap); decrease on 32-bit Unix (real fds, avoid exhaustion). See PR #12495.
...
static leveldb::Options GetOptions(size_t nCacheSize)
{
    ...
    options.info_log = new CTKNCLevelDBLogger();
    ...
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}
{
    ...
    // PathToString() safe for leveldb open: POSIX passes bytes to ::open(); Windows converts UTF-8→UTF-16 for ::CreateFileW.

...
{
/** Logger is intentionally leaked: global destruction order is undefined, and a late destructor calling LogInfo after logger destruction would core dump. ~Logger() is never called, so Logger subclasses must have implicitly-defined destructors. */
...
namespace BCLog {
    /** Escape control characters (except '\n') in C syntax to prevent terminal control code injection; escapes rather than removes for troubleshooting. */

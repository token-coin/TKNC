...
static bool LoadLLamaFunctions(const std::string& dll_path) {
    ...
    // Load multi-GPU enumeration APIs from llama.dll (optional — may not exist in older builds)
...
bool LLamaDLL::Load(const std::string& dll_path) {
    ...
    //
    // Dependency chain: llama.dll → ggml.dll → ggml-base.dll → libomp140.x86_64.dll
    ...
    std::string dll_dir = resolved_dll_path;
    {
        ...
        if (pos != std::string::npos) {
            ...
            // No backslash — might be bare filename or forward-slash path
            pos = dll_dir.rfind('/');
            if (pos != std::string::npos) {
                dll_dir = dll_dir.substr(0, pos);
            }
    ...
    for (int i = 0; dep_order[i]; i++) {
        ...
        if (hDep) {
            ...
            // Don't abort — some deps might be optional or already loaded
        }
    }
        ...
        //   NVIDIA → CUDA first (best performance), then Vulkan fallback
        //   AMD    → Vulkan first (broader compatibility), then CUDA fallback
...
#ifdef _WIN32
        {
            ...
            if (RegOpenKeyExA(HKEY_LOCAL_MACHINE,
                ...
                RegCloseKey(hKey);
                // Prefer NVIDIA dGPU over AMD iGPU — critical for mixed-GPU systems
    ...
    // Step 2: Call llama_backend_init() — this loads backends into llama.dll's registry
    if (pfn_llama_backend_init) {
    ...
    if (!cpu_in_llama && pfn_llama_ggml_backend_register) {
        ...
        if (h_cpu) {
            ...
            // NOTE: do NOT FreeLibrary(h_cpu) — the backend needs to stay loaded
        } else {
    ...
    // ================================================================
    // GPU DEVICE DETECTION — enumerate ALL GPU devices for multi-GPU
    //
    // [GPU-FIX v2] Corrected device type priority based on ggml-backend.h:
    //   GGML_BACKEND_DEVICE_TYPE_CPU   = 0
    //   GGML_BACKEND_DEVICE_TYPE_GPU   = 1  (dGPU — dedicated, always preferred)
    //   GGML_BACKEND_DEVICE_TYPE_IGPU  = 2  (iGPU — integrated, fallback only)
    ...
    if (pfn_llama_ggml_backend_reg_count && pfn_llama_ggml_backend_dev_by_type) {
        ...
        // Strategy 2: Fallback — use dev_by_type for each type (older llama.dll)
        ...
        // Sort: dGPU(1) first, then iGPU(2), then others — within same type, larger VRAM first

...
LLMInference::GenerationResult LLMInference::Generate(const std::string& prompt, const std::string& system_prompt) {
 ...
 // Decode prompt in chunks based on actual n_batch of the context.
 int actual_n_batch = dll.n_batch(ctx);
 if (actual_n_batch <= 0) actual_n_batch = 2048;
 for (int offset = 0; offset < n_tokens; offset += actual_n_batch) {
     int chunk = std::min(actual_n_batch, n_tokens - offset);
     int32_t decode_ret = dll.decode(ctx, dll.batch_get_one(tokens.data() + offset, chunk));
     if (decode_ret != 0) {
         result.error = "Initial decode failed at offset " + std::to_string(offset) + ", ret=" + std::to_string(decode_ret);
         std::cerr << "[LLM] ERROR: " << result.error << std::endl;
         dll.free(ctx);
         return result;
     }
 ...
 std::string response;
 int n_decoded = 0;
 int32_t ret;  // Used in the generation loop below

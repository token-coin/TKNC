...
struct SpendingLimit {
 ...
 CAmount consumed_tknc; // TKNC already consumed (billed)
 CAmount spending_limit; // Remaining spending limit (total - consumed)

 // === Sliding window grace period billing ===
 // transferred_tknc: total TKNC transferred via TransferFromWallet (tx created+broadcast)
 // confirmed_tknc: total TKNC confirmed on-chain (in a block, depth >= 1)
 // pending_txids: semicolon-separated "txid:amount" pairs awaiting confirmation
 // Rule: allow inference if (consumed_tknc - confirmed_tknc) < 2 * COIN (1 TKNC grace)
 CAmount transferred_tknc; // TKNC transferred (tx created)
 CAmount confirmed_tknc; // TKNC confirmed on-chain
 std::string pending_txids; // Pending transfer txids: "txid1:amount1;txid2:amount2;..."
 ...
 transferred_tknc(0), confirmed_tknc(0),
 ...
 obj.transferred_tknc, obj.confirmed_tknc, obj.pending_txids,

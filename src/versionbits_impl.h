...
#ifndef TKN_VERSIONBITS_IMPL_H
#define TKN_VERSIONBITS_IMPL_H
...
public:
    ...
    /** Numerical statistics of an in-progress BIP9 softfork in the period including pindex. If provided, signalling_blocks is set to true/false based on whether each block in the period signalled. */
    BIP9Stats GetStateStatisticsFor(const CBlockIndex* pindex, std::vector<bool>* signalling_blocks = nullptr) const;
    /** State for pindex A based on parent pindexPrev B. Applies any state transition if conditions are present. Caches state from first block of period. */

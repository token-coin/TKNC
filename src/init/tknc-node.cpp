...
namespace init {
namespace {
const char* EXE_NAME = "tknc-node";

class TKNCNodeInit : public interfaces::Init
{
public:
    TKNCNodeInit(node::NodeContext& node, const char* arg0)
        : m_node(node),
...
{
    auto init = std::make_unique<init::TKNCNodeInit>(node, argc > 0 ? argv[0] : "");
    // Check if tknc-node is being invoked as an IPC server. If so, then

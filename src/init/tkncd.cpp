...
namespace init {
namespace {
const char* EXE_NAME = "tkncd";

class TknckernelInit : public interfaces::Init
{
public:
    TknckernelInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::TknckernelInit>(node);
}
} // namespace interfaces

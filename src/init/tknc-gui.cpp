...
namespace init {
namespace {
const char* EXE_NAME = "tknc-gui";

class TknckernelGuiInit : public interfaces::Init
{
public:
    TknckernelGuiInit(const char* arg0) : m_ipc(interfaces::MakeIpc(EXE_NAME, arg0, *this))
    {
        InitContext(m_node);
        m_node.init = this;
    }
    ...
    // tknc-gui accepts -ipcbind option even though it does not use it
    // directly. It just returns true here to accept the option because
    // tknc-node accepts the option, and tknc-gui accepts all tknc-node
...
{
    return std::make_unique<init::TknckernelGuiInit>(argc > 0 ? argv[0] : "");
}
} // namespace interfaces
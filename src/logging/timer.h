...
#ifndef TKN_LOGGING_TIMER_H
#define TKN_LOGGING_TIMER_H
...
public:
    ...
    std::string LogMsg(const std::string& msg)
    {
        ...
        if constexpr (std::is_same_v<TimeType, std::chrono::microseconds>) {
            return strprintf("%s: %s (%ius)", m_prefix, msg, Ticks<std::chrono::microseconds>(duration));

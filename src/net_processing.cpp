...
#include <rpc/escrow_rpc.h> // WriteSpendingLimitFromP2P for ESCROWSYNC handler, CanAffordInference, CheckAndDeductEscrow
#include <billing/billing_receipt.h> // BillingReceipt for post-inference billing
#include <util/moneystr.h> // FormatMoney for billing log messages
 ...
 const std::atomic<bool>& interruptMsgProc)
{
 ...
 // API key has insufficient balance. This prevents free inference via P2P APIREQ.
 // Without this check, any connected peer could get free inference by sending
 // APIREQ messages, bypassing billing entirely.
 if (!request.api_key.empty()) {
     EnsureEscrowForAPIKey(request.api_key);
     std::string balance_error;
     if (!CanAffordInference(request.api_key, balance_error)) {
         LogWarning("[P2P-API-BILLING] Inference REFUSED for peer=%d, api_key=%s...: %s",
                     pfrom.GetId(), request.api_key.substr(0, 8).c_str(), balance_error.c_str());
         APIResponse err_response;
         err_response.request_id = request.request_id;
         err_response.content = "Error: Inference refused — " + balance_error;
         err_response.tokens_used = 0;
         err_response.cost = 0;
         std::vector<uint8_t> response_data = err_response.Serialize();
         MakeAndPushMessage(pfrom, MessageTypes::APIRESP, response_data);
     ...
     // this client. Prevents billing bypass where client modifies their node code.
     std::string miner_pay_error;
     if (!CheckMinerReceivedPayment(request.api_key, miner_pay_error)) {
         LogWarning("[P2P-API-MINER-PAY] Inference REFUSED for peer=%d, api_key=%s...: %s",
                     pfrom.GetId(), request.api_key.substr(0, 8).c_str(), miner_pay_error.c_str());
         APIResponse err_response;
         err_response.request_id = request.request_id;
         err_response.content = "Error: Inference refused — " + miner_pay_error;
         err_response.tokens_used = 0;
         err_response.cost = 0;
         std::vector<uint8_t> response_data = err_response.Serialize();
         MakeAndPushMessage(pfrom, MessageTypes::APIRESP, response_data);
     ...
     // SECURITY: Refuse APIREQ without API key — no free inference.
     LogWarning("[P2P-API-BILLING] Refused APIREQ from peer=%d: no API key provided", pfrom.GetId());
     APIResponse err_response;
     err_response.request_id = request.request_id;
     err_response.content = "Error: API key required for inference. Generate one via tknc_createapikey.";
     err_response.tokens_used = 0;
     err_response.cost = 0;
     std::vector<uint8_t> response_data = err_response.Serialize();
     MakeAndPushMessage(pfrom, MessageTypes::APIRESP, response_data);
 ...
 // even when local_result.success is true (backend parses body, not HTTP status).
 bool relay_miner_error = (local_result.content.empty()
     || local_result.content.find("[Error:") != std::string::npos
     || local_result.content.find("[Miner returned") != std::string::npos
     || local_result.content.find("[Inference unavailable:") != std::string::npos
     || local_result.content.find("[LLM inference error:") != std::string::npos
     || local_result.content.find("[FATAL:") != std::string::npos);

 if (local_result.success && !request.api_key.empty() && local_result.tokens_used > 0 && !relay_miner_error) {
     // Track served tokens for miner-side independent payment verification
     TrackMinerServedTokens(request.api_key, local_result.tokens_used);

     BillingReceipt receipt;
     if (CheckAndDeductEscrow(request.api_key, local_result.tokens_used, receipt)) {
         LogInfo("[P2P-API-BILLING] Billing SUCCESS: peer=%d, tokens=%lld, cost=%s TKNC, consumed=%s TKNC",
                 pfrom.GetId(), (long long)local_result.tokens_used,
                 FormatMoney(receipt.cost_tknc).c_str(),
                 FormatMoney(receipt.consumed_tknc).c_str());
     } else {
         LogWarning("[P2P-API-BILLING] Billing FAILED: peer=%d, tokens=%lld — escrow exhausted/invalid",
                    pfrom.GetId(), (long long)local_result.tokens_used);
     }
 } else if (local_result.success && relay_miner_error) {
     LogWarning("[P2P-API-BILLING] SKIPPED billing: miner returned error/empty response "
                "(peer=%d, tokens=%lld, content_prefix=%.60s)",
                pfrom.GetId(), (long long)local_result.tokens_used,
                local_result.content.substr(0, 60).c_str());
 }

...
bool CNetAddr::SetNetFromBIP155Network(uint8_t possible_bip155_net, size_t address_size)
{
    ...
    // Silently drop unknown network ids (maybe from the future).
    return false;
}
...
void CNetAddr::SetLegacyIPv6(std::span<const uint8_t> ipv6)
{
    ...
    if (HasPrefix(ipv6, IPV4_IN_IPV6_PREFIX)) {
        ...
        // TORv2-in-IPv6 (unsupported): unserialize as !IsValid() to ignore but continue stream.
        m_net = NET_IPV6;
...
/** Create an "internal" address representing a name or FQDN for AddrMan tracking. */

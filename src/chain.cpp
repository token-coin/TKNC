...
int static inline GetSkipHeight(int height) {
    ...
    // Jump-back height: any value < height works; this expression performs well (≤110 steps for 2**18 blocks).
...
arith_uint256 GetBitsProof(uint32_t bits)
{
    ...
    if (fNegative || fOverflow || bnTarget == 0)
        return 0;
    if (bnTarget >= ~arith_uint256(0))
        return 1;
    arith_uint256 result = (~bnTarget / (bnTarget + 1)) + 1;
    if (result == 0)
        result = 1;
    return result;
}

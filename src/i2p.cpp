...
/** Decode I2P-style Base64 string; throws std::runtime_error on failure. */
...
/** Derive .b32.i2p address from binary I2P destination; throws on failure. */
...
/** Derive .b32.i2p address from Base64 I2P destination; throws on failure. */
...
bool Session::Accept(Connection& conn)
{
    ...
    while (!m_interrupt->interrupted()) {
        ...
        try {
            ...
            // Unexpected reply (e.g. "STREAM STATUS RESULT=I2P_ERROR"): session damaged, close it.
                                               ...
                                               bool check_result_ok) const
{
    ...
    // I2P router may take minutes to reply; RecvUntilTerminator checks m_interrupt frequently.
...
{
    // Use SIGNATURE_TYPE=7 (EdDSA_SHA512_Ed25519); i2pd <2.24.0 needs numeric form. See geti2p.net/spec/common-structures#key-certificates.
...
{
    // I2P destination: 387 bytes + cert length at bytes 385-386. See geti2p.net/spec/common-structures#destination.

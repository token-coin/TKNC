...
/** Parse a DER-like ECPrivateKey encoding (SEC 1 C.4): SEQUENCE length is 1-2 octets (not strict DER), privateKey is zero-filled to 32 bytes, trailing data is ignored. out32 must be ≥32 bytes. */
...
/** Serialize to DER encoding of ECPrivateKey (SEC 1 C.4), including optional parameters and publicKey fields. seckey buffer must be ≥CKey::SIZE bytes; *seckeylen is set to bytes used. */
...
bool SigHasLowR(const secp256k1_ecdsa_signature* sig)
{
    ...
    // In DER, high bit of the first byte indicates sign; ensuring it's 0 (< 0x80) avoids prepending a 0x00 byte, keeping sig < 71 bytes.
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    unsigned char rnd[8];
    std::string str = "TKNC key verification\n";
    GetRandBytes(rnd);
...
{
    static const unsigned char hashkey[] = {'T','K','N','C',' ','s','e','e','d'};
    std::vector<unsigned char, secure_allocator<unsigned char>> vout(64);

...
#ifndef TKN_UINT256_H
#define TKN_UINT256_H
...
public:
    ...
    /** Lexicographic ordering. @note Does NOT match the ordering on the corresponding \ref base_uint::CompareTo, which starts comparing from the end. */
    constexpr int Compare(const base_blob& other) const { return std::memcmp(m_data.data(), other.m_data.data(), WIDTH); }

    friend constexpr bool operator==(const base_blob& a, const base_blob& b) { return a.Compare(b) == 0; }
    friend constexpr bool operator<(const base_blob& a, const base_blob& b) { return a.Compare(b) < 0; }

    /** @name Hex representation. The hex representation used by GetHex(), ToString(), and FromHex() is unusual: it shows bytes of the base_blob in reverse order, so a 4-byte blob {0x12, 0x34, 0x56, 0x78} is represented as "78563412". This matches little-endian numeric formatting (most significant digit first) so ArithToUint256(num).GetHex() can display an arith_uint256 num value as a number without needing number parsing/formatting functions. @{*/
...
namespace detail {
/** Writes the hex string (in reverse byte order) into a new uintN_t object and only returns a value iff all checks pass: input length is uintN_t::size()*2 and all characters are hex. */
...
/** Like FromHex(std::string_view str), but allows an "0x" prefix and pads the input with leading zeroes if shorter than uintN_t::size()*2. Designed for user input. */
template <class uintN_t>
...
/** 160-bit opaque blob. @note This type is called uint160 for historical reasons only. It is an opaque blob of 160 bits and has no integer operations. */

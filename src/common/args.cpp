...
const char * const TKNC_CONF_FILENAME = "tknc.conf";
const char * const TKNC_SETTINGS_FILENAME = "settings.json";

ArgsManager gArgs;

/** Interpret a string argument as a boolean. LocaleIndependentAtoi<int>() returns 0 for non-numeric values like "foo", so non-integer user input here always returns false. This means -foo=false does what users expect, but -foo=true is well defined yet probably not what they expected. For extensive discussion see PR12713. */
...
/** Parse "name", "section.name", "noname", "section.noname" settings keys. @note Where an option was negated can be later checked using the IsArgNegated() method. One use case is to disable options that are not normally boolean (e.g. using -nodebuglogfile to request that debug log output is not sent to any file at all). */
...
// Default constructor and destructor are not inline, so code instantiating this class doesn't need to #include class definitions for all members (e.g. m_settings has an internal dependency on univalue).
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef __APPLE__
        // At the first time when a user gets the "App downloaded from the internet" warning, and clicks the Open button, macOS passes a unique process serial number (PSN) as -psn_... command-line argument, which we filter out.
        if (key.starts_with("-psn_")) continue;
#endif

        if (key == "-") break; //tknc-tx using stdin
        ...
        // Unknown command line options and command line options with dot characters (which are returned from InterpretKey with nonempty section strings) are not valid.
...
{
    fs::path settings = GetPathArg("-settings", TKNC_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Data dir is always "data" subdirectory at the same level as the exe. e.g. D:\TKNC\build\bin\tkncd.exe → D:\TKNC\build\bin\data\. This is deterministic regardless of cwd or launch method.
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    auto get_net = [&](const std::string& arg) {
        ...
        common::SettingsValue value = common::GetSetting(m_settings, /* section= */ "", SettingName(arg),
            /* ignore_default_section_config= */ false, /*ignore_nonpersistent=*/false, /* get_chain_type= */ true);
...
void ArgsManager::LogArgs() const
{
    ...
    for (const auto& setting : m_settings.rw_settings) {
        LogDebug(BCLog::ALL, "Setting file arg: %s = %s\n", setting.first, setting.second.write());
    }

...
CBloomFilter::CBloomFilter(const unsigned int nElements, const double nFPRate, const unsigned int nTweakIn, unsigned char nFlagsIn) :
    /** The ideal size for a bloom filter with given nElements and false positive rate is nElements * log(fp rate) / ln(2)^2. We ignore filter parameters which would create a bloom filter larger than the protocol limits. */
    vData(std::min((unsigned int)(-1  / LN2SQUARED * nElements * log(nFPRate)), MAX_BLOOM_FILTER_SIZE * 8) / 8),
    /** The ideal number of hash functions is filter size * ln(2) / number of elements. We ignore filter parameters which would create a bloom filter with more hash functions than the protocol limits. See https://en.wikipedia.org/wiki/Bloom_filter for an explanation of these formulas. */
...
bool CBloomFilter::IsRelevantAndUpdate(const CTransaction& tx)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        // Match if the filter contains any arbitrary script data element in any scriptPubKey in tx. If this matches, also add the specific output that was matched — this means clients don't have to update the filter themselves when a new relevant tx is discovered in order to find spending transactions, which avoids round-tripping and race conditions.

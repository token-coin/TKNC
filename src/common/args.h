...
#ifndef TKN_COMMON_ARGS_H
#define TKN_COMMON_ARGS_H
...
extern const char * const TKNC_CONF_FILENAME;
extern const char * const TKNC_SETTINGS_FILENAME;

// Return true if -datadir option points to a valid directory or is not specified.
bool CheckDataDirOption(const ArgsManager& args);

/** Most paths passed as configuration arguments are treated as relative to the datadir if they are not absolute. @param args Parsed arguments and settings. @param path The path to be conditionally prefixed with datadir. @param net_specific Use network specific datadir variant. @return The normalized path. */
...
public:
    ...
    enum Flags : uint32_t {
        ...
        // ALLOW_BOOL/ALLOW_INT/ALLOW_STRING/ALLOW_LIST = 0x02/0x04/0x08/0x10  -- unimplemented, draft implementation in #16545
        DISALLOW_NEGATION = 0x20, //!< disallow -nofoo syntax
        DISALLOW_ELISION = 0x40,  //!< disallow -foo syntax that doesn't assign any value

        DEBUG_ONLY = 0x100,
        /* Some options would cause cross-contamination if values for mainnet were used while running on regtest/testnet (or vice-versa). Setting them as NETWORK_ONLY ensures that sharing a config file between mainnet and regtest/testnet won't cause problems due to these parameters by accident. */
        NETWORK_ONLY = 0x200,
...
public:
    ...
    /** Get data directory path with appended network identifier. @return Absolute path on success, otherwise an empty path when a non-directory path would be returned. */
    fs::path GetDataDirNet() const EXCLUSIVE_LOCKS_REQUIRED(!cs_args);

    /** Clear cached directory paths. */
    void ClearPathCache() EXCLUSIVE_LOCKS_REQUIRED(!cs_args);

    /** Return a vector of strings of the given argument. @param strArg Argument to get (e.g. "-foo"). @return command-line arguments. */
    ...
    /** Returns the appropriate chain type from the program arguments. @return ChainType::MAIN by default; raises runtime error if an invalid combination, or unknown chain is given. */
    ChainType GetChainType() const EXCLUSIVE_LOCKS_REQUIRED(!cs_args);

    /** Returns the appropriate chain type string from the program arguments. @return ChainType::MAIN string by default; raises runtime error if an invalid combination is given. */
    ...
    /** Check CLI command args. @throws std::runtime_error when multiple CLI_COMMAND arguments are specified. */
...
private:
    ...
    /** Get data directory path. @param net_specific Append network identifier to the returned path. @return Absolute path on success, otherwise an empty path when a non-directory path would be returned. */
    fs::path GetDataDir(bool net_specific) const EXCLUSIVE_LOCKS_REQUIRED(cs_args);

    /** Return -regtest/-signet/-testnet/-testnet4/-chain= setting as a ChainType enum if a recognized chain type was set, or as a string if an unrecognized chain name was set. Raises an exception if an invalid combination of flags was provided. */
...
/** Format a string to be used as option description in help messages. @param option Option message (e.g. "-rpcuser=<user>"). @param message Option description (e.g. "Username for JSON-RPC connections"). @return the formatted string. */
std::string HelpMessageOpt(const std::string& option, const std::string& message);

#endif // TKN_COMMON_ARGS_H

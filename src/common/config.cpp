...
{
    {
        ...
        m_settings.ro_config.clear();
        m_config_sections.clear();
        const auto conf_val = GetPathArg_("-conf", TKNC_CONF_FILENAME);
    // Use exe root directory (not datadir) for config file so it survives data/ cleanup
    m_config_path = (conf_val.is_absolute() || conf_val.empty()) ? conf_val : fsbridge::AbsPathJoin(GetExeDir(), conf_val);
    }
    ...
    // ok to not have a config file — but auto-create one for zero-config experience
    if (!stream.good() && !IsArgSet("-conf") && !conf_path.empty()) {
        ...
        if (out.good()) {
            out << DEFAULT_CONF;
            out.close();
            LogInfo("Auto-generated default config file: %s", fs::PathToString(conf_path));
            stream = std::ifstream{conf_path.std_path()};
        }
    }
    if (stream.good()) {

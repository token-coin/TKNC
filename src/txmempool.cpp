...
void CTxMemPool::addNewTransaction(CTxMemPool::txiter newit)
{
    ...
    // Update cachedInnerUsage to include contained transaction's usage. (When updating the entry for in-mempool parents, memory usage will be further updated.)
    ...
    // Don't bother worrying about child transactions of this one. Normal case of a new transaction arriving is that there can't be any children (they'd be orphans). Exception: tx entering that used to be in a block—disconnect block logic calls UpdateTransactionsFromBlock to clean up.
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    if (reason != MemPoolRemovalReason::BLOCK && m_opts.signals) {
        // Notify clients that a transaction has been removed from the mempool for any reason except being included in a block. Clients interested in transactions included in blocks can subscribe to the BlockConnected notification.
...
void CTxMemPool::removeRecursive(const CTransaction &origTx, MemPoolRemovalReason reason)
{
    ...
    if (origit != mapTx.end()) {
        ...
        // When recursively removing but origTx isn't in the mempool, be sure to remove any descendants that are in the pool. Can happen during chain re-orgs if origTx isn't re-accepted into the mempool for any reason.
...
void CTxMemPool::check(const CCoinsViewCache& active_coins_tip, int64_t spendheight) const
{
    ...
    for (const auto& it : score_with_topo) {
        // GetSortedScoreWithTopology() contains the same chunks as the feerate diagram. Chunk boundaries unknown, but can check that there are points at which they match cumulative fee and weight. Feerate diagram should never get behind the current transaction size totals.
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            // We are iterating through the mempool entries sorted topologically and by mining score. All parents must have been checked before their children and their coins added to the mempoolDuplicate coins cache.
...
{
    // Return true if hasha should be considered sooner than hashb, namely when: a is not in the mempool but b is, or both are in the mempool but a is sorted before b in the total mempool ordering (which takes dependencies and (chunk) feerates into account).
    LOCK(cs);
...
std::optional<Coin> CCoinsViewMemPool::GetCoin(const COutPoint& outpoint) const
{
    ...
    // If an entry in the mempool exists, always return that one—guaranteed to never conflict with the underlying cache, and cannot have pruned entries (contains full transactions). First checking the underlying cache risks returning a pruned entry instead.
...
{
    LOCK(cs);
    // Use ChangeSet interface to check whether the cluster count limits would be violated. Note: the changeset is destroyed when it goes out of scope.
...
void CTxMemPool::TrimToSize(size_t sizelimit, std::vector<COutPoint>* pvNoSpendsRemaining) {
    ...
    while (!mapTx.empty() && DynamicMemoryUsage() > sizelimit) {
        ...
        // Set new mempool min fee to the feerate of removed set + "minimum reasonable fee rate" (some value under which we consider txn to have 0 fee). This way, we don't allow txn to enter mempool with feerate equal to txn which were removed with no block in between.
...
std::vector<CTxMemPool::txiter> CTxMemPool::GatherClusters(const std::vector<Txid>& txids) const
{
    ...
    for (auto txid : txids) {
        auto it = mapTx.find(txid);
        if (it != mapTx.end()) {
            // TxGraph::GetCluster returns results in graph order, which is deterministic (as long as graph not being modified).

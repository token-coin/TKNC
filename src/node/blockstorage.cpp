...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block, CBlockIndex*& best_header)
{
    ...
    arith_uint256 blockWork = GetBlockProof(*pindexNew);
    if (blockWork == 0) {
        blockWork = arith_uint256(1);
        LogError("WARNING: GetBlockProof returned 0 for block %s, using minimum work\n", pindexNew->GetBlockHash().ToString());
    }
    pindexNew->nChainWork = (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + blockWork;
    if (pindexNew->pprev && pindexNew->nChainWork < pindexNew->pprev->nChainWork) {
        LogError("WARNING: nChainWork decreased at height %d, correcting\n", pindexNew->nHeight);
        pindexNew->nChainWork = pindexNew->pprev->nChainWork + blockWork;
    }
...
bool BlockManager::LoadBlockIndex(const std::optional<uint256>& snapshot_blockhash)
{
    ...
    for (CBlockIndex* pindex : vSortedByHeight) {
        ...
        arith_uint256 loadBlockWork = GetBlockProof(*pindex);
        if (loadBlockWork == 0) {
            loadBlockWork = arith_uint256(1);
            LogError("WARNING: GetBlockProof returned 0 for block %s during load, using minimum work\n", pindex->GetBlockHash().ToString());
        }
        pindex->nChainWork = (pindex->pprev ? pindex->pprev->nChainWork : 0) + loadBlockWork;
        if (pindex->pprev && pindex->nChainWork < pindex->pprev->nChainWork) {
            LogError("WARNING: nChainWork decreased at height %d during load, correcting\n", pindex->nHeight);
            pindex->nChainWork = pindex->pprev->nChainWork + loadBlockWork;
        }
...
{
    LogInfo("ImportBlocks: entered, m_blockfiles_indexed=%d", chainman.m_blockman.m_blockfiles_indexed);
    ...
    LogInfo("ImportBlocks: calling ActivateBestChains...");
    if (auto result = chainman.ActivateBestChains(); !result) {
        chainman.GetNotifications().fatalError(util::ErrorString(result));
    }
    // Also activate the best chain for the normal (non-assumeutxo) chainstate.
    // ActivateBestChains() only handles assumeutxo chainstates, so the normal
    // chainstate needs to be activated separately to set the tip block.
    LogInfo("ImportBlocks: calling ActivateBestChain for active chainstate...");
    {
        BlockValidationState state;
        if (!chainman.ActiveChainstate().ActivateBestChain(state, nullptr)) {
            LogWarning("ImportBlocks: ActivateBestChain failed for active chainstate: %s", state.ToString());
        } else {
            LogInfo("ImportBlocks: ActivateBestChain succeeded for active chainstate");
        }
    }
    LogInfo("ImportBlocks: done");

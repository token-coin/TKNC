...
void UpdatePSBTOutput(const SigningProvider& provider, PartiallySignedTransaction& psbt, int index)
{
    ...
    // Construct a would-be spend to fill sigdata with metadata; no private keys are needed (provider can be a HidingSigningProvider).
...
PSBTError SignPSBTInput(const SigningProvider& provider, PartiallySignedTransaction& psbt, int index, const PrecomputedTransactionData* txdata, std::optional<int> sighash,  SignatureData* out_sigdata, bool finalize)
{
    ...
    if (input.non_witness_utxo) {
        ...
        utxo = input.witness_utxo;
        // Witness UTXO data can't be verified against the spent output, so a witness signature is required for safety.
        require_witness_sig = true;
    } else {
        return PSBTError::MISSING_INPUTS;
    }

    // Resolve sighash type: field and parameter must match if both provided; default to SIGHASH_DEFAULT for taproot, SIGHASH_ALL otherwise.
    ...
    // Set the PSBT sighash field only for non-default values; DEFAULT aliases to ALL for non-taproot inputs during signing.
    ...
    if (sigdata.witness) {
        ...
        // non_witness_utxo removal depends on the entire transaction's input mix, so it's left to the caller (FillPSBT).
    }
...
void RemoveUnnecessaryTransactions(PartiallySignedTransaction& psbtx)
{
    ...
    for (unsigned int i = 0; i < psbtx.inputs.size(); ++i) {
        ...
        // SIGHASH_ANYONECANPAY prevents dropping non_witness_utxos; absent sighash field implies DEFAULT or ALL.
...
{
    // Finalize input signatures by combining partial signatures into a final script if needed.
    bool complete = true;
